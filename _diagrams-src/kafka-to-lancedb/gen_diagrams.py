...
Renders (SVG is what the docs page embeds; HTML files are local previews):
  ...
  stage-file-process.svg     per-message dispatch: one process_message per message,
...
# peach = component), including the two-column dispatch rows.
...
def flow_svg():
    ...
    s.append(f'<text x="82" y="{y+11}" dominant-baseline="central" font-family="{MONO}" '
             ...
             f'COCOINDEX · KAFKA → LANCEDB FLOW (CONSUME)</text>')
    y += 30 + 28

    s.append(fnode(CX, y, NODE_W, "logic", "Source", "kafka.topic_as_map",
                   "subscribe topic · keyed messages · offsets managed"))
    y += FNODE_H
    s.append(vdown(CX, y)); y += VGAP

    s.append(fnode(CX, y, NODE_W, "data", "Data", "messages", "one Message per Kafka record"))
    y += FNODE_H
    s.append(vdown(CX, y)); y += VGAP

    # ── processing component ──
    s.append(fcomp(CX, y, COMP_W, COMP_H, "Processing component",
                   "mount_each → process_message (per message)"))
    iy = y + COMP_PAD_T
    s.append(fnode(CX, iy, INNER_W, "logic", "Transform · parse & dispatch",
                   "json.loads(value)",
                   'shape check: "sku" → Product · "emp_id" → Employee'))
    iy += FNODE_H
    s.append(vdown(CX, iy)); iy += VGAP
    s.append(fnode(CX - ROW_W / 2 - 15, iy, ROW_W, "target", "Target state",
                   "declare_row(Product)", "products table · pk = sku"))
    s.append(fnode(CX + ROW_W / 2 + 15, iy, ROW_W, "target", "Target state",
                   "declare_row(Employee)", "employees table · pk = emp_id"))
    y += COMP_H
    s.append(vdown(CX, y)); y += VGAP

    TROW_W = 470  # two bottom targets, 30px gap
    s.append(fnode(CX - TROW_W / 2 - 15, y, TROW_W, "logic", "Target",
                   "LanceDB · products", "upsert by sku"))
    s.append(fnode(CX + TROW_W / 2 + 15, y, TROW_W, "logic", "Target",
                   "LanceDB · employees", "upsert by emp_id"))
    y += FNODE_H
