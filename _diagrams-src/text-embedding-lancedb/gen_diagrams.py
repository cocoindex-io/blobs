#!/usr/bin/env python3
"""V1 diagrams for the `text-embedding-lancedb` example.

Adapted from _diagrams-src/text-embedding/gen_diagrams.py, fitted to
examples/text_embedding_lancedb/main.py — same chunk/embed pipeline, LanceDB
as the target instead of Postgres:
  walk_dir -> files -> [process_file: RecursiveSplitter(markdown)
  -> for each chunk: embed -> declare_row(DocEmbedding)] -> LanceDB (embedded).
...
.coco{{position:absolute;top:60px;right:78px;font-size:60px;}}
  ...
  <div>
    <h1>Semantic Search<br>with <em>LanceDB</em></h1>
    <div class="sub">Chunk Markdown, embed each chunk, and store the vectors in an embedded LanceDB table — no server, incremental, in plain async Python.</div>
  </div>
...
.sw.comp{{background:color-mix(in oklab,{PEACH} 13%,{CREAM});border-color:{CORAL};border-radius:8px;}}
  ...
  <div class="eyebrow"><span class="dot"></span>CocoIndex &middot; Text embedding flow &middot; LanceDB</div>
  ...
  <div class="col">
    ...
    {ARROW}
    {node("logic", "Target", "LanceDB &middot; embedded", "doc_embeddings table &middot; ./lancedb_data &middot; no server")}
  </div>
