...
def flow_svg():
    ...
    s.append(f'<text x="82" y="{y+11}" dominant-baseline="central" font-family="{MONO}" '
             ...
             f'COCOINDEX · TEXT EMBEDDING FLOW · TURBOPUFFER</text>')
    y += 30 + 28
    ...
    s.append(fnode(CX, sy, SUB_INNER_W, "target", "Target state",
                   "declare_row → turbopuffer.Row",
                   "id · vector · attributes (filename, text, offsets)"))
    y += COMP_H
    s.append(vdown(CX, y)); y += VGAP

    s.append(fnode(CX, y, NODE_W, "logic", "Target", "Turbopuffer · namespace",
                   "TextEmbedding namespace · ANN vector index", mono=False))
    y += FNODE_H
