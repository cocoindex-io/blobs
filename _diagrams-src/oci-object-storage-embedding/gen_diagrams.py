...
def flow_svg():
    ...
    s.append(f'<text x="82" y="{y+11}" dominant-baseline="central" font-family="{MONO}" '
             ...
             f'COCOINDEX · OCI OBJECT STORAGE EMBEDDING FLOW</text>')
    y += 30 + 28

    s.append(fnode(CX, y, NODE_W, "logic", "Source", "oci_object_storage.list_objects",
                   "OCI bucket · *.md · optional live via OCI Streaming"))
    y += FNODE_H
    s.append(vdown(CX, y)); y += VGAP

    s.append(fnode(CX, y, NODE_W, "data", "Data", "files", "one OCIFile per Markdown object"))
    y += FNODE_H
    s.append(vdown(CX, y)); y += VGAP

    # ── processing component ──
    s.append(fcomp(CX, y, COMP_W, COMP_H, "Processing component",
                   "mount_each → process_file (per object, memoized)"))
    iy = y + COMP_PAD_T
    ...
    s.append(fnode(CX, y, NODE_W, "logic", "Target", "Postgres · pgvector",
                   "oci_object_storage_doc_embeddings table", mono=False))
    y += FNODE_H
