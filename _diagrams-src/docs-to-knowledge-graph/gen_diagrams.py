...
Renders (SVG is what the docs page embeds; HTML files are local previews):
  ...
  stage-phase1.svg   per-doc extraction components highlighted
  stage-phase2.svg   single graph-building pass highlighted
  schema.png         graph schema (raster only — the docs page keeps the PNG)
...
def edge(x1, y1, x2, y2, label=None, *, lx=None, ly=None):
    ...
    if label:
        s += (f'<text x="{lx}" y="{ly}" text-anchor="middle" '
              f'font-family="{MONO}" font-size="15" font-weight="600" '
...
def flow_svg():
    ...
    s.append(f'<text x="82" y="{y+11}" dominant-baseline="central" font-family="{MONO}" '
             ...
             f'COCOINDEX · DOCS → KNOWLEDGE GRAPH FLOW</text>')
    y += 30 + 28

    s.append(fnode(CX, y, NODE_W, "logic", "Source", "localfs.walk_dir",
                   "Markdown docs folder · *.md / *.mdx"))
    y += FNODE_H
    s.append(vdown(CX, y)); y += VGAP

    s.append(fnode(CX, y, NODE_W, "data", "Data", "files", "one File per document"))
    y += FNODE_H
    s.append(vdown(CX, y)); y += VGAP

    # ── Phase 1 ──
    s.append(fcomp(CX, y, COMP_W, COMP_H, "Phase 1 · Processing component",
                   "use_mount → process_file (per doc, memoized)"))
    iy = y + COMP_PAD_T
    s.append(fnode(CX, iy, INNER_W, "logic", "Transform · LLM extraction",
                   "instructor + LiteLLM",
                   "extract_summary · extract_relationships"))
    iy += FNODE_H
    s.append(vdown(CX, iy)); iy += VGAP
    s.append(fnode(CX - ROW_W / 2 - 15, iy, ROW_W, "target", "Target state",
                   "Document node", "declare_record · filename · title · summary"))
    s.append(fnode(CX + ROW_W / 2 + 15, iy, ROW_W, "data", "Data · carried forward",
                   "DocTriples", "(subject, predicate, object) × N"))
    y += COMP_H
    s.append(vdown(CX, y)); y += VGAP

    # ── Phase 2 ──
    s.append(fcomp(CX, y, COMP_W, COMP_H, "Phase 2 · Processing component",
                   "mount → build_graph (one pass, all docs)"))
    iy = y + COMP_PAD_T
    s.append(fnode(CX, iy, INNER_W, "logic", "Transform",
                   "dedupe entities · stable ids",
                   "generate_id(subject, predicate, object)"))
    iy += FNODE_H
    s.append(vdown(CX, iy)); iy += VGAP
    ...
    s.append(fnode(CX, y, NODE_W, "logic", "Target", "Neo4j",
                   "property graph · Document · Entity · edges"))
    y += FNODE_H
...
# schema stays raster-only: the docs page embeds schema.png.
sw_, sh_, sbody = schema_svg()
(OUT_DIR / "schema.html").write_text(page(sw_, sh_, sbody, pad=34))
print("wrote schema.html")

if "--render" in sys.argv:
    chrome = "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome"
    for name, (ww, hh) in {**sizes, "schema": (sw_ + 68, sh_ + 68)}.items():
        subprocess.run([
