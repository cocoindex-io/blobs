#!/usr/bin/env python3
"""V1 diagrams for the `image-search-colpali` example.

Adapted from _diagrams-src/image-search, fitted to
examples/image_search_colpali/pipeline.py:
  localfs.walk_dir(*.jpg/png, live) -> files -> [process_file (per file, memoized):
  ColPali image encoder -> multi-vector (bag of patch vectors) ->
  declare_point(id, vector, filename)] -> Qdrant collection (MaxSim multivector).
  Query side: ColPali text encoder -> bag of token vectors -> Qdrant MaxSim search.

Renders:
  cover.png              social/hero cover
  flow-v1.png            full vertical pipeline (hero)
  stage-file-process.png per-image fan-out: one process_file component per image

The search-results screenshot is reused from the example README, not generated here.
...
def cover_html():
    ...
    pipeline = sep.join([chip("Images"), chip("ColPali"), chip("Multi-vectors"), chip("Qdrant · MaxSim")])
...
.coco{{position:absolute;top:60px;right:78px;font-size:60px;}}
  ...
  <div>
    <h1>Image Search<br>with <em>ColPali</em></h1>
    <div class="sub">Embed images and queries into a bag of patch vectors with ColPali, store them in Qdrant, and rank with late-interaction MaxSim.</div>
  </div>
...
.sw.comp{{background:color-mix(in oklab,{PEACH} 13%,{CREAM});border-color:{CORAL};border-radius:8px;}}
  ...
  <div class="eyebrow"><span class="dot"></span>CocoIndex &middot; ColPali image search flow (indexing)</div>
  ...
  <div class="col">
    ...
    <div class="comp">
      ...
      <div class="col">
        {node("logic", "Transform · ColPali image encoder", "embed_image_bytes", "image → multi-vector (bag of 128-d patch vectors)")}
        {ARROW}
        {node("target", "Target state · one per image", "declare_point → Qdrant", "id (uuid5 of path) · multivector · payload {filename}")}
      </div>
    </div>

    {ARROW}
    {node("logic", "Target", "Qdrant", "ImageSearchColpali collection · MaxSim multivector · cosine")}
  </div>
...
def fanout_svg(*, hl_fn=True):
    ...
    PADX = 30; FILE_W = 140; FILE_H = 52; GAPF = 40
    PC_PADX = 28; XFM_W = 318; XFM_H = 92; GAPX = 34; OUT_W = 262; OUT_H = 56
    ...
    s.append(logic(TGT_X, APP_Y, TGT_W, APP_H, ["Qdrant"], size=26, sub="MaxSim multivector"))

    for cy, fname in [(160, "cat1.jpeg"), (340, "giraffe.jpg")]:
        s.append(bind(SRC[0] + SRC[2], cy, file_x))
        s.append(data(file_x, cy - FILE_H/2, FILE_W, FILE_H, [fname], hl=hl_fn, size=19))
        s.append(arrow(file_x + FILE_W, cy, pc_x))
        s.append(pc(pc_x, cy - PCH/2, PCW, PCH, "process_file", hl=hl_fn))
        s.append(logic(xfm_x, cy - XFM_H/2 + 10, XFM_W, XFM_H,
                       ["ColPali multi-vector embed"], hl=hl_fn, size=22))
        s.append(arrow(xfm_x + XFM_W, cy, out_x))
        s.append(bullet(out_x, cy - OUT_H/2, OUT_W, OUT_H,
                        ["Qdrant point"], hl=hl_fn, size=18))
...
if "--render" in sys.argv:
    ...
    subprocess.run([
        chrome, "--headless=new", "--hide-scrollbars", "--no-sandbox",
        "--force-color-profile=srgb", "--virtual-time-budget=8000",
        f"--screenshot={OUT_DIR / 'flow-v1.png'}", "--window-size=1240,1360",
