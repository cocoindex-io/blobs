...
Renders (SVG is what the docs page embeds; HTML files are local previews):
  ...
  stage-file-process.svg  per-image fan-out: one process_file component per image
...
def flow_svg():
    ...
    s.append(f'<text x="82" y="{y+11}" dominant-baseline="central" font-family="{MONO}" '
             ...
             f'COCOINDEX · COLPALI IMAGE SEARCH FLOW (INDEXING)</text>')
    y += 30 + 28

    s.append(fnode(CX, y, NODE_W, "logic", "Source", "localfs.walk_dir",
                   "Local filesystem · *.jpg / *.png · watches for changes (live=True)"))
    y += FNODE_H
    s.append(vdown(CX, y)); y += VGAP

    s.append(fnode(CX, y, NODE_W, "data", "Data", "files", "one FileLike per image"))
    y += FNODE_H
    s.append(vdown(CX, y)); y += VGAP

    # ── processing component ──
    s.append(fcomp(CX, y, COMP_W, COMP_H, "Processing component",
                   "mount_each → process_file (per image, memoized)"))
    iy = y + COMP_PAD_T
    s.append(fnode(CX, iy, INNER_W, "logic", "Transform · ColPali image encoder",
                   "embed_image_bytes", "image → multi-vector (bag of 128-d patch vectors)"))
    iy += FNODE_H
    s.append(vdown(CX, iy)); iy += VGAP
    s.append(fnode(CX, iy, INNER_W, "target", "Target state · one per image",
                   "declare_point → Qdrant", "id (uuid5 of path) · multivector · payload {filename}"))
    y += COMP_H
    s.append(vdown(CX, y)); y += VGAP

    s.append(fnode(CX, y, NODE_W, "logic", "Target", "Qdrant",
                   "ImageSearchColpali collection · MaxSim multivector · cosine"))
    y += FNODE_H
...
DIAGRAMS = {
    "flow-v1": (flow_svg, 0),
    "stage-file-process": (fanout_svg, 34),
}
