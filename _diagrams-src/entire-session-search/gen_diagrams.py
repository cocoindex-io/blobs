...
Renders (SVG is what the docs page embeds; HTML files are local previews):
  ...
  stage-file-process.svg  per-file routing + embed highlighted
  stage-main-function.svg files + processing components highlighted (mount_each)

Everything but the cover is vector: text stays live text with a web-safe font
...
def flow_svg():
    ...
    COMP_W = W - 120
    NODE_W = 760
    WIDE_NODE_W = 900  # the source node carries a long file-routing subtitle
    INNER_W = COMP_W - 2 * COMP_PAD_X
    SUB_W = INNER_W
    SUB_INNER_W = SUB_W - 2 * SUB_PAD_X

    SUB_H = SUB_PAD_T + FNODE_H + VGAP + FNODE_H + SUB_PAD_B
    COMP_H = (COMP_PAD_T + FNODE_H + VGAP + FNODE_H + VGAP + SUB_H + VGAP
              + FNODE_H + COMP_PAD_B)
    ...
    s.append(f'<text x="82" y="{y+11}" dominant-baseline="central" font-family="{MONO}" '
             ...
             f'COCOINDEX · ENTIRE SESSION SEARCH FLOW</text>')
    y += 30 + 28

    s.append(fnode(CX, y, WIDE_NODE_W, "logic", "Source", "localfs.walk_dir",
                   "Entire checkpoints · full.jsonl / prompt.txt / context.md / metadata.json · live=True"))
    y += FNODE_H
    s.append(vdown(CX, y)); y += VGAP

    s.append(fnode(CX, y, NODE_W, "data", "Data", "files",
                   "one FileLike per checkpoint file"))
    y += FNODE_H
    s.append(vdown(CX, y)); y += VGAP
    ...
    iy = y + COMP_PAD_T
    s.append(fnode(CX, iy, INNER_W, "logic", "Transform · route by filename",
                   "parse_transcript / RecursiveSplitter / parse metadata",
                   "transcript turns · prompt · context chunks · metadata row"))
    iy += FNODE_H
    s.append(vdown(CX, iy)); iy += VGAP
    s.append(fnode(CX, iy, INNER_W, "data", "Data", "text + metadata",
                   "ChunkInput per turn / chunk · structured session fields"))
    iy += FNODE_H
    s.append(vdown(CX, iy)); iy += VGAP

    s.append(fsubcomp(CX, iy, SUB_W, SUB_H, "coco.map → process_chunk (per text chunk)"))
    sy = iy + SUB_PAD_T
    ...
    s.append(fnode(CX, sy, SUB_INNER_W, "target", "Target state",
                   "declare_row → SessionEmbeddingRow",
                   "id · checkpoint · content_type · role · text · embedding"))
    iy += SUB_H
    s.append(vdown(CX, iy)); iy += VGAP
    s.append(fnode(CX, iy, INNER_W, "target", "Target state",
                   "declare_row → SessionMetadataRow",
                   "checkpoint · prompt_summary · total_tokens · files_touched"))
    y += COMP_H
    s.append(vdown(CX, y)); y += VGAP

    s.append(fnode(CX, y, NODE_W, "logic", "Target", "Postgres · pgvector",
                   "session_embeddings + session_metadata · sequential scan"))
    y += FNODE_H
