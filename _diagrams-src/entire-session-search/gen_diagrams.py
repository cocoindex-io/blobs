#!/usr/bin/env python3
"""V1 diagrams for the `entire-session-search` example.

Adapted from _diagrams-src/pdf-embedding/gen_diagrams.py, fitted to
examples/entire_session_search/main.py:
  walk_dir(Entire checkpoints) -> files -> [process_file (per file, memoized):
  route by name (full.jsonl / prompt.txt / context.md / metadata.json)
  -> embed text via coco.map process_chunk -> declare_row(SessionEmbeddingRow);
  metadata.json -> declare_row(SessionMetadataRow)] -> two Postgres tables
  (pgvector, sequential scan — no vector index).

Renders:
  cover.png              social/hero cover
  flow-v1.png            full vertical pipeline (hero)
  stage-file-process.png per-file routing + embed highlighted
  stage-main-function.png files + processing components highlighted (mount_each)
...
def cover_html():
    ...
    pipeline = sep.join([chip("Entire"), chip("Transcripts"), chip("Chunks"),
...
.sub{{font-size:27px;color:{MUTED};line-height:1.4;max-width:900px;margin-top:6px;}}
...
.coco{{position:absolute;top:60px;right:78px;font-size:60px;}}
  ...
  <div>
    <h1>Search your<br><em>AI coding sessions</em></h1>
    <div class="sub">Read Entire checkpoints — transcripts, prompts, context summaries — embed them, and store the vectors in Postgres for semantic search. Incremental, in plain async Python.</div>
  </div>
...
.sw.comp{{background:color-mix(in oklab,{PEACH} 13%,{CREAM});border-color:{CORAL};border-radius:8px;}}
  ...
  <div class="eyebrow"><span class="dot"></span>CocoIndex &middot; Entire session search flow</div>

  <div class="col">
    {node("logic", "Source", "localfs.walk_dir", "Entire checkpoints &middot; full.jsonl / prompt.txt / context.md / metadata.json &middot; live=True")}
    {ARROW}
    {node("data", "Data", "files", "one FileLike per checkpoint file")}
    {ARROW}

    <div class="comp">
      <div class="comp-cap">Processing Component &middot; <span class="reg">mount_each &rarr; process_file (per file, memoized)</span></div>
      <div class="col">
        {node("logic", "Transform &middot; route by filename", "parse_transcript / RecursiveSplitter / parse metadata", "transcript turns &middot; prompt &middot; context chunks &middot; metadata row")}
        {ARROW}
        {node("data", "Data", "text + metadata", "ChunkInput per turn / chunk &middot; structured session fields")}
        {ARROW}
        <div class="subcomp">
          <div class="subcomp-cap">coco.map &rarr; process_chunk (per text chunk)</div>
          <div class="col">
            {node("logic", "Transform", "SentenceTransformerEmbedder.embed", "chunk text &rarr; embedding (384-d vector)")}
            {ARROW}
            {node("target", "Target state", "declare_row &rarr; SessionEmbeddingRow", "id &middot; checkpoint &middot; content_type &middot; role &middot; text &middot; embedding")}
          </div>
        </div>
        {ARROW}
        {node("target", "Target state", "declare_row &rarr; SessionMetadataRow", "checkpoint &middot; prompt_summary &middot; total_tokens &middot; files_touched")}
      </div>
    </div>

    {ARROW}
    {node("logic", "Target", "Postgres &middot; pgvector", "session_embeddings + session_metadata &middot; sequential scan")}
  </div>
...
def fanout_svg(*, hl_fn=False, hl_file=False, hl_pc=False):
    ...
    PC_PADX = 28; XFM_W = 320; XFM_H = 92; GAPX = 36; OUT_W = 188; OUT_H = 56
    ...
    s = [DEFS]
    s.append(logic(*SRC, ["Entire", "checkpoints"], size=25, sub="walk_dir · live"))
    s.append(app(APP_X, APP_Y, APP_W, APP_H, "CocoIndex App"))
    s.append(logic(TGT_X, APP_Y, TGT_W, APP_H, ["Postgres"], size=26, sub="pgvector"))

    for cy, fname, route, out in [
        (160, "full.jsonl", "route · parse · embed", ["SessionEmbedding", "rows"]),
        (340, "metadata.json", "route · parse meta", ["SessionMetadata", "row"]),
    ]:
        s.append(bind(SRC[0] + SRC[2], cy, file_x))
        s.append(data(file_x, cy - FILE_H/2, FILE_W, FILE_H, [fname], hl=hl_file, size=18))
...
if "--render" in sys.argv:
    ...
    subprocess.run([
        chrome, "--headless=new", "--hide-scrollbars", "--no-sandbox",
        "--force-color-profile=srgb", "--virtual-time-budget=8000",
        f"--screenshot={OUT_DIR / 'flow-v1.png'}", "--window-size=1240,2040",
