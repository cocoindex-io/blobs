#!/usr/bin/env python3
"""V1 diagrams for the `google-drive-embedding` example.

Adapted from _diagrams-src/pdf-embedding/gen_diagrams.py, fitted to
examples/gdrive_text_embedding/main.py:
  GoogleDriveSource(root_folder_ids) -> files -> [process_file (per file,
...
def cover_html():
    ...
    pipeline = sep.join([chip("Google Drive"), chip("Chunks"),
...
.coco{{position:absolute;top:60px;right:78px;font-size:60px;}}
  ...
  <div>
    <h1>Semantic Search<br>over <em>Google Drive</em></h1>
    <div class="sub">Read documents from a Drive folder, chunk, embed, and store the vectors in Postgres — incremental, in plain async Python.</div>
  </div>
