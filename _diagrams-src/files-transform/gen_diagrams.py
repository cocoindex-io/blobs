...
Renders:
  ...
  stage-file-process.png per-file transform + file target highlighted
...
.sw.comp{{background:color-mix(in oklab,{PEACH} 13%,{CREAM});border-color:{CORAL};border-radius:8px;}}
  ...
  <div class="eyebrow"><span class="dot"></span>CocoIndex &middot; Files transform flow</div>
  ...
  <div class="col">
    ...
    <div class="comp">
      ...
      <div class="col">
        {node("logic", "Transform &middot; markdown-it-py", "MarkdownIt.render", "Markdown text &rarr; HTML string", hl=True)}
        {ARROW}
        {node("data", "Data", "html", "rendered HTML &middot; outname from path parts")}
        {ARROW}
        {node("target", "Target state", "localfs.declare_file", "outdir / name.html &middot; create_parent_dirs=True")}
      </div>
    </div>

    {ARROW}
    {node("logic", "Target", "Local filesystem", "output_html/ folder &middot; one .html per source .md")}
  </div>
...
if "--render" in sys.argv:
    ...
    subprocess.run([
        chrome, "--headless=new", "--hide-scrollbars", "--no-sandbox",
        "--force-color-profile=srgb", "--virtual-time-budget=8000",
        f"--screenshot={OUT_DIR / 'flow-v1.png'}", "--window-size=1240,1420",
