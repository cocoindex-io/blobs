#!/usr/bin/env python3
"""V1 diagrams for the `files-transform` example.

Adapted from _diagrams-src/pdf-embedding/gen_diagrams.py, fitted to
examples/files_transform/main.py:
  walk_dir(*.md, live=True) -> files -> [process_file (per file, memoized):
  markdown_it.render -> HTML -> localfs.declare_file] -> local output_html/ folder.
...
Renders (SVG is what the docs page embeds; HTML files are local previews):
  ...
  stage-file-process.svg     per-file transform + file target highlighted
...
def cover_html():
    ...
    pipeline = sep.join([chip("Markdown folder"), chip("render"),
                         chip("HTML"), chip("output folder")])
...
.coco{{position:absolute;top:60px;right:78px;font-size:60px;}}
  ...
  <div>
    <h1>Transform a<br><em>Folder of Files</em></h1>
    <div class="sub">Watch a directory of Markdown, render each file to HTML with markdown-it-py, and write the outputs to a local folder — incremental, in plain async Python.</div>
  </div>
...
def fanout_svg(*, hl_fn=False, hl_file=False, hl_pc=False):
    ...
    s = [DEFS]
    s.append(logic(*SRC, ["Markdown", "folder"], size=26, sub="walk_dir · live"))
    s.append(app(APP_X, APP_Y, APP_W, APP_H, "CocoIndex App"))
    s.append(logic(TGT_X, APP_Y, TGT_W, APP_H, ["output_html/"], size=24, sub="local folder"))
...
def flow_svg():
    ...
    s.append(f'<text x="82" y="{y+11}" dominant-baseline="central" font-family="{MONO}" '
             ...
             f'COCOINDEX · FILES TRANSFORM FLOW</text>')
    y += 30 + 28

    s.append(fnode(CX, y, NODE_W, "logic", "Source", "localfs.walk_dir",
                   "Local filesystem · *.md · watches for changes (live=True)"))
    y += FNODE_H
    ...
    iy = y + COMP_PAD_T
    s.append(fnode(CX, iy, INNER_W, "logic", "Transform · markdown-it-py",
                   "MarkdownIt.render", "Markdown text → HTML string", hl=True))
    iy += FNODE_H
    s.append(vdown(CX, iy)); iy += VGAP
    s.append(fnode(CX, iy, INNER_W, "data", "Data", "html",
                   "rendered HTML · outname from path parts"))
    iy += FNODE_H
    s.append(vdown(CX, iy)); iy += VGAP
    s.append(fnode(CX, iy, INNER_W, "target", "Target state", "localfs.declare_file",
                   "outdir / name.html · create_parent_dirs=True"))
    y += COMP_H
    s.append(vdown(CX, y)); y += VGAP

    s.append(fnode(CX, y, NODE_W, "logic", "Target", "Local filesystem",
                   "output_html/ folder · one .html per source .md"))
    y += FNODE_H
