#!/usr/bin/env python3
"""V1 diagrams for the `pdf-embedding` example.
...
examples/pdf_embedding/main.py:
  ...
  pdf_to_markdown (docling, GPU runner) -> RecursiveSplitter(markdown)
  -> coco.map process_chunk: embed -> declare_row(PdfEmbedding)] -> Postgres
  (pgvector, sequential scan — no vector index).

Renders:
  cover.png              social/hero cover
  flow-v1.png            full vertical pipeline (hero)
  stage-file-process.png per-file transform chain highlighted
...
def cover_html():
    ...
    pipeline = sep.join([chip("PDF"), chip("Markdown"), chip("Chunks"),
...
.coco{{position:absolute;top:60px;right:78px;font-size:60px;}}
  ...
  <div>
    <h1>Semantic Search<br>over <em>PDFs</em></h1>
    <div class="sub">Convert PDFs to Markdown with docling, chunk, embed, and store the vectors in Postgres — incremental, in plain async Python.</div>
  </div>
...
.sw.comp{{background:color-mix(in oklab,{PEACH} 13%,{CREAM});border-color:{CORAL};border-radius:8px;}}
  ...
  <div class="eyebrow"><span class="dot"></span>CocoIndex &middot; PDF embedding flow</div>
  ...
  <div class="col">
    ...
    <div class="comp">
      ...
      <div class="col">
        {node("logic", "Transform &middot; docling &middot; GPU runner", "pdf_to_markdown", "PDF bytes &rarr; Markdown &middot; coco.fn.as_async(runner=GPU)")}
        {ARROW}
        ...
        <div class="subcomp">
          ...
          <div class="col">
            ...
            {ARROW}
            {node("target", "Target state", "declare_row &rarr; PdfEmbedding", "id &middot; filename &middot; text &middot; embedding &middot; char offsets")}
          </div>
        </div>
      </div>
    </div>
    ...
    {ARROW}
    {node("logic", "Target", "Postgres &middot; pgvector", "pdf_embeddings table &middot; sequential scan")}
  </div>
...
def fanout_svg(*, hl_fn=False, hl_file=False, hl_pc=False):
    ...
    for cy, fname in [(160, "attention.pdf"), (340, "bert.pdf")]:
...
if "--render" in sys.argv:
    ...
    subprocess.run([
        chrome, "--headless=new", "--hide-scrollbars", "--no-sandbox",
        "--force-color-profile=srgb", "--virtual-time-budget=8000",
        f"--screenshot={OUT_DIR / 'flow-v1.png'}", "--window-size=1240,1820",
