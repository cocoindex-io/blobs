#!/usr/bin/env python3
"""V1 diagrams for the `text-embedding-qdrant` example.

Adapted from _diagrams-src/text-embedding/gen_diagrams.py, fitted to
examples/text_embedding_qdrant/main.py — the Semantic Search 101 flow with
Qdrant as the target instead of Postgres/pgvector:
  walk_dir(*.md) -> files -> [process_file (per file, memoized):
  RecursiveSplitter(markdown) -> coco.map process_chunk: embed ->
  declare_point(PointStruct)] -> Qdrant collection.
...
.coco{{position:absolute;top:60px;right:78px;font-size:60px;}}
  ...
  <div>
    <h1>Semantic Search<br>with <em>Qdrant</em></h1>
    <div class="sub">Chunk Markdown, embed each chunk locally, and upsert the vectors into a managed Qdrant collection — incremental, in plain async Python.</div>
  </div>
...
.sw.comp{{background:color-mix(in oklab,{PEACH} 13%,{CREAM});border-color:{CORAL};border-radius:8px;}}
  ...
  <div class="eyebrow"><span class="dot"></span>CocoIndex &middot; Text embedding flow &middot; Qdrant</div>
  ...
  <div class="col">
      ...
      <div class="col">
        ...
        <div class="subcomp">
          ...
          <div class="col">
            ...
            {ARROW}
            {node("target", "Target state", "declare_point &rarr; PointStruct", "id &middot; vector &middot; payload (filename, text, offsets)")}
          </div>
        </div>
      </div>
    </div>
    ...
    {ARROW}
    {node("logic", "Target", "Qdrant &middot; collection", "TextEmbedding &middot; managed upserts (gRPC)")}
  </div>
