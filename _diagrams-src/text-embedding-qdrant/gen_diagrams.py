...
def flow_svg():
    ...
    s.append(f'<text x="82" y="{y+11}" dominant-baseline="central" font-family="{MONO}" '
             ...
             f'COCOINDEX · TEXT EMBEDDING FLOW · QDRANT</text>')
    y += 30 + 28
    ...
    s.append(fnode(CX, sy, SUB_INNER_W, "target", "Target state",
                   "declare_point → PointStruct",
                   "id · vector · payload (filename, text, offsets)"))
    y += COMP_H
    s.append(vdown(CX, y)); y += VGAP

    s.append(fnode(CX, y, NODE_W, "logic", "Target", "Qdrant · collection",
                   "TextEmbedding · managed upserts (gRPC)", mono=False))
    y += FNODE_H
