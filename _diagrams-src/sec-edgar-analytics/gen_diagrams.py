...
</style></head><body><div class="eyebrow"><span class="dot"></span>CocoIndex &middot; SEC EDGAR analytics flow</div><div class="col">
<div class="row">{node("logic","Source","localfs.walk_dir","10-K filings &middot; *.txt")}{node("logic","Source","localfs.walk_dir","company facts &middot; *.json")}</div>{ARROW}
<div class="comp"><div class="comp-cap">Processing Component &middot; <span class="reg">process_filing / process_facts (per file, memoized)</span></div><div class="col">
{node("logic","Transform","scrub_pii + render","SSN/phone/email redacted; JSON &rarr; text")}{ARROW}
{node("logic","Transform &middot; RecursiveSplitter","chunk","1000 chars, 200 overlap")}{ARROW}
<div class="row">{node("logic","Transform &middot; embed","SentenceTransformer","chunk &rarr; vector")}{node("logic","Transform","extract_topics","RISK:* / TOPIC:* tags")}</div>{ARROW}
{node("target","Target state &middot; one per chunk","declare_row &rarr; FilingChunk","text &middot; embedding &middot; topics &middot; metadata")}</div></div>{ARROW}
<div class="row">{node("target","Doris &middot; ANN vector index","l2_distance","semantic search")}{node("target","Doris &middot; INVERTED index","MATCH_ANY","keyword search")}</div>
