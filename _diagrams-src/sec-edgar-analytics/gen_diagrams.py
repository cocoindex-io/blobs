#!/usr/bin/env python3
"""V1 diagrams for the `sec-edgar-analytics` example.

Fitted to examples/sec_edgar_analytics/main.py:
  walk_dir(10-K *.txt) + walk_dir(company facts *.json) ->
  [process_filing / process_facts (per file, memoized): scrub_pii + render ->
  RecursiveSplitter chunk -> embed + extract_topics -> declare_row
  FilingChunk] -> Apache Doris (ANN vector index + INVERTED index).
...
def cover_html():
    ...
    pipe=sep.join([chip("Filings"),chip("Chunk + Embed"),chip("Topics"),chip("Doris")])
...
<div><h1>SEC Filing<br><em>Hybrid Search</em></h1><div class="sub">Scrub, chunk, embed and tag multi-format SEC filings into Apache Doris with vector + full-text indexes for hybrid retrieval.</div></div>
...
def flow_svg():
    ...
    ROW_W = (INNER_W - 30) / 2
    OUT_ROW_W = (COMP_W - 30) / 2

    COMP_H = (COMP_PAD_T + FNODE_H + VGAP + FNODE_H + VGAP + FNODE_H + VGAP
              + FNODE_H + COMP_PAD_B)
    ...
    s.append(f'<text x="82" y="{y+11}" dominant-baseline="central" font-family="{MONO}" '
             ...
             f'COCOINDEX · SEC EDGAR ANALYTICS FLOW</text>')
    y += 30 + 28

    s.append(fnode(CX - OUT_ROW_W / 2 - 15, y, OUT_ROW_W, "logic", "Source",
                   "localfs.walk_dir", "10-K filings · *.txt"))
    s.append(fnode(CX + OUT_ROW_W / 2 + 15, y, OUT_ROW_W, "logic", "Source",
                   "localfs.walk_dir", "company facts · *.json"))
    y += FNODE_H
    s.append(vdown(CX, y)); y += VGAP

    # ── processing component ──
    s.append(fcomp(CX, y, COMP_W, COMP_H, "Processing component",
                   "process_filing / process_facts (per file, memoized)"))
    iy = y + COMP_PAD_T
    s.append(fnode(CX, iy, INNER_W, "logic", "Transform", "scrub_pii + render",
                   "SSN/phone/email redacted; JSON → text"))
    iy += FNODE_H
    s.append(vdown(CX, iy)); iy += VGAP
    s.append(fnode(CX, iy, INNER_W, "logic", "Transform · RecursiveSplitter",
                   "chunk", "1000 chars, 200 overlap"))
    iy += FNODE_H
    s.append(vdown(CX, iy)); iy += VGAP
    s.append(fnode(CX - ROW_W / 2 - 15, iy, ROW_W, "logic", "Transform · embed",
                   "SentenceTransformer", "chunk → vector"))
    s.append(fnode(CX + ROW_W / 2 + 15, iy, ROW_W, "logic", "Transform",
                   "extract_topics", "RISK:* / TOPIC:* tags"))
    iy += FNODE_H
    s.append(vdown(CX, iy)); iy += VGAP
    s.append(fnode(CX, iy, INNER_W, "target", "Target state · one per chunk",
                   "declare_row → FilingChunk",
                   "text · embedding · topics · metadata"))
    y += COMP_H
    s.append(vdown(CX, y)); y += VGAP

    s.append(fnode(CX - OUT_ROW_W / 2 - 15, y, OUT_ROW_W, "target",
                   "Doris · ANN vector index", "l2_distance", "semantic search"))
    s.append(fnode(CX + OUT_ROW_W / 2 + 15, y, OUT_ROW_W, "target",
                   "Doris · INVERTED index", "MATCH_ANY", "keyword search"))
    y += FNODE_H
