...
def flow_svg():
    ...
    s.append(f'<text x="82" y="{y+11}" dominant-baseline="central" font-family="{MONO}" '
             ...
             f'COCOINDEX · IMAGE SEARCH FLOW (INDEXING)</text>')
    y += 30 + 28
    ...
    iy = y + COMP_PAD_T
    s.append(fnode(CX, iy, INNER_W, "logic", "Transform · CLIP image encoder",
                   "embed_image_bytes", "image → embedding (CLIP ViT-L/14, 768-d)"))
    iy += FNODE_H
    s.append(vdown(CX, iy)); iy += VGAP
    s.append(fnode(CX, iy, INNER_W, "target", "Target state · one per image",
                   "declare_point → Qdrant", "id (uuid5 of path) · vector · payload {filename}"))
    y += COMP_H
    s.append(vdown(CX, y)); y += VGAP

    s.append(fnode(CX, y, NODE_W, "logic", "Target", "Qdrant",
                   "ImageSearch collection · cosine"))
    y += FNODE_H
