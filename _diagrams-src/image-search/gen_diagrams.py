#!/usr/bin/env python3
"""V1 diagrams for the `image-search` example.

Adapted from _diagrams-src/csv-to-kafka and text-embedding generators, fitted to
examples/image_search/pipeline.py:
  localfs.walk_dir(*.jpg/png, live) -> files -> [process_file (per file, memoized):
  CLIP image encoder -> declare_point(id, vector, filename)] -> Qdrant collection
  (cosine). Query side: CLIP text encoder -> Qdrant search.
...
def cover_html():
    ...
    pipeline = sep.join([chip("Images"), chip("CLIP"), chip("Vectors"), chip("Qdrant")])
...
.coco{{position:absolute;top:60px;right:78px;font-size:60px;}}
  ...
  <div>
    <h1>Search Images<br>by <em>Text</em></h1>
    <div class="sub">Embed images and queries into one CLIP space, store the vectors in Qdrant, and search your photos in plain English.</div>
  </div>
...
.sw.comp{{background:color-mix(in oklab,{PEACH} 13%,{CREAM});border-color:{CORAL};border-radius:8px;}}
  ...
  <div class="eyebrow"><span class="dot"></span>CocoIndex &middot; Image search flow (indexing)</div>
  ...
  <div class="col">
    ...
    <div class="comp">
      ...
      <div class="col">
        {node("logic", "Transform · CLIP image encoder", "embed_image_bytes", "image → embedding (CLIP ViT-L/14, 768-d)")}
        {ARROW}
        {node("target", "Target state · one per image", "declare_point → Qdrant", "id (uuid5 of path) · vector · payload {filename}")}
      </div>
    </div>

    {ARROW}
    {node("logic", "Target", "Qdrant", "ImageSearch collection · cosine")}
  </div>
...
def fanout_svg(*, hl_fn=True):
    ...
    for cy, fname in [(160, "cat1.jpeg"), (340, "giraffe.jpg")]:
        ...
        s.append(logic(xfm_x, cy - XFM_H/2 + 10, XFM_W, XFM_H,
                       ["CLIP image embed"], hl=hl_fn, size=23))
...
if "--render" in sys.argv:
    ...
    subprocess.run([
        chrome, "--headless=new", "--hide-scrollbars", "--no-sandbox",
        "--force-color-profile=srgb", "--virtual-time-budget=8000",
        f"--screenshot={OUT_DIR / 'flow-v1.png'}", "--window-size=1240,1320",
