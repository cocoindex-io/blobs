...
Renders (SVG is what the docs page embeds; HTML files are local previews):
  ...
  stage-file-process.svg     per-file fan-out: one process_file component per audio file
...
def flow_svg():
    ...
    s.append(f'<text x="82" y="{y+11}" dominant-baseline="central" font-family="{MONO}" '
             ...
             f'COCOINDEX · AUDIO TO TEXT FLOW</text>')
    y += 30 + 28

    s.append(fnode(CX, y, NODE_W, "logic", "Source", "localfs.walk_dir",
                   "Local filesystem · recursive · audio extensions"))
    y += FNODE_H
    s.append(vdown(CX, y)); y += VGAP

    s.append(fnode(CX, y, NODE_W, "data", "Data", "files", "one File per audio file"))
    y += FNODE_H
    s.append(vdown(CX, y)); y += VGAP
    ...
    iy = y + COMP_PAD_T
    s.append(fnode(CX, iy, INNER_W, "logic", "Transform", "LiteLLMTranscriber.transcribe",
                   "audio bytes → transcript text (whisper-1)", hl=True))
    iy += FNODE_H
    s.append(vdown(CX, iy)); iy += VGAP
    s.append(fnode(CX, iy, INNER_W, "target", "Target state",
                   "declare_row → AudioTranscription",
                   "filename (primary key) · text", hl=True))
    y += COMP_H
    s.append(vdown(CX, y)); y += VGAP

    s.append(fnode(CX, y, NODE_W, "logic", "Target", "Postgres",
                   "audio_transcriptions table · one row per file"))
    y += FNODE_H
