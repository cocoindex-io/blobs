#!/usr/bin/env python3
"""V1 diagrams for the `meeting-notes-to-knowledge-graph` example.

Adapted from _diagrams-src/docs-to-knowledge-graph/gen_diagrams.py, fitted to
examples/meeting_notes_graph_neo4j/main.py — a THREE-phase pipeline:

  google_drive source -> [phase 1: process_file (per note, memoized): split +
  LLM extract -> Meeting + Task nodes, DECIDED edges, raw person names carried
  forward] -> [phase 2: resolve_persons (one pass): embed + LLM pair resolver
  -> canonical name map] -> [phase 3: create_person_relations (one pass):
  Person nodes + ATTENDED / ASSIGNED_TO edges using resolved names] -> Neo4j.

Renders (SVG is what the docs page embeds; HTML files are local previews):
  flow-v1.svg        full vertical pipeline (hero), all three phases
  stage-phase1.svg   per-note extraction components highlighted
  stage-phase2.svg   single person entity-resolution pass highlighted
  stage-phase3.svg   single person-relations pass highlighted
...
The graph schema and the Neo4j-browser result are reused screenshots, not
generated here.
...
def phase1_svg():
    ...
    PC_PADX = 26; XFM_W = 290; XFM_H = 92; GAPX = 36; OUT_W = 282; PCH = 184
    ...
    s = [DEFS]
    s.append(logic(*SRC, ["Notes", "folder"], size=26, sub="Google Drive"))
    s.append(app(APP_X, APP_Y, APP_W, APP_H, "CocoIndex App"))
    s.append(logic(TGT_X, APP_Y, TGT_W, APP_H, ["GraphDB"], size=26, sub="graph"))

    for cy, fname in [(182, "TeamA.md"), (372, "TeamB.md")]:
        ...
        s.append(logic(xfm_x, cy - XFM_H/2 + 12, XFM_W, XFM_H,
                       ["split + LLM extract"], size=23, sub="memoized"))
        s.append(arrow(xfm_x + XFM_W, cy - 26, out_x))
        s.append(bullet(out_x, cy - 26 - 27, OUT_W, 54, ["Meeting · Task nodes"], size=17))
        s.append(arrow(xfm_x + XFM_W, cy + 44, out_x))
        s.append(data(out_x, cy + 44 - 25, OUT_W, 50, ["MeetingExtraction ⟶ ②③"], size=16))
...
# ─────────────────────────── stage: phase 2 entity resolution ───────────────────────────
def phase2_svg():
    APP_Y = 40; APP_H = 360
    SRC = (40, APP_Y, 210, APP_H)
    APP_X = 290
    PADX = 32; IN_W = 188; IN_H = 56; GAPF = 42
    PC_PADX = 26; XFM_W = 340; XFM_H = 104; GAPX = 38; OUT_W = 300; PCH = 196
    PCW = PC_PADX + XFM_W + GAPX + OUT_W + PC_PADX
    APP_W = PADX + IN_W + GAPF + PCW + PADX
    W = APP_X + APP_W + 40; H = APP_Y + APP_H + 32
    ...
    s = [DEFS]
    s.append(logic(*SRC, ["raw person", "names × all notes"], size=22, sub="from phase ①"))
    s.append(app(APP_X, APP_Y, APP_W, APP_H, "CocoIndex App"))

    s.append(bind(SRC[0] + SRC[2], CY, in_x))
    s.append(data(in_x, CY - IN_H/2, IN_W, IN_H, ["set[str]"], size=21))
    s.append(arrow(in_x + IN_W, CY, pc_x))
    s.append(pc(pc_x, CY - PCH/2, PCW, PCH, "resolve_persons (one pass)", hl=True))
    s.append(logic(xfm_x, CY - XFM_H/2, XFM_W, XFM_H,
                   ["Embed + LLM resolve"], size=23, sub="arctic-embed · LlmPairResolver"))
    s.append(arrow(xfm_x + XFM_W, CY, out_x))
    s.append(data(out_x, CY - 33, OUT_W, 66, ["ResolvedEntities", "canonical map ⟶ ③"], size=18))
    return W, H, "".join(s)


# ─────────────────────────── stage: phase 3 person relations ───────────────────────────
def phase3_svg():
    APP_Y = 40; APP_H = 400
    SRC = (40, APP_Y, 210, APP_H)
    APP_X = 290
    PADX = 32; IN_W = 188; IN_H = 60; GAPF = 42
    PC_PADX = 26; XFM_W = 320; XFM_H = 104; GAPX = 36; OUT_W = 300; PCH = 300
    PCW = PC_PADX + XFM_W + GAPX + OUT_W + PC_PADX
    APP_W = PADX + IN_W + GAPF + PCW + PADX
    TGT_W = 184
    TGT_X = APP_X + APP_W + 40
    ...
    s = [DEFS]
    s.append(logic(*SRC, ["MeetingExtraction", "+ ResolvedEntities"], size=20,
                   sub="from phases ① ②"))
    s.append(app(APP_X, APP_Y, APP_W, APP_H, "CocoIndex App"))
    s.append(logic(TGT_X, APP_Y, TGT_W, APP_H, ["GraphDB"], size=26, sub="graph"))

    s.append(bind(SRC[0] + SRC[2], CY, in_x))
    s.append(data(in_x, CY - IN_H/2, IN_W, IN_H, ["meetings", "+ persons"], size=20))
    s.append(arrow(in_x + IN_W, CY, pc_x))
    s.append(pc(pc_x, CY - PCH/2, PCW, PCH, "create_person_relations", hl=True))
    s.append(logic(xfm_x, CY - XFM_H/2, XFM_W, XFM_H,
                   ["Aggregate · resolved"], size=23, sub="organizer flag · dedup"))
    for dy, label in [(-64, "Person nodes"), (8, "ATTENDED"), (80, "ASSIGNED_TO")]:
...
# ─────────────────────────── flow (hero), as SVG ───────────────────────────
# Vector port of flow_html(): same vertical pipeline, same shape language
# (rounded = logic, square-ish = data, flag = target state, peach = component).
...
def flow_svg():
    ...
    # Component heights: caption block + stacked content + bottom pad.
    ...
    s.append(f'<text x="82" y="{y+11}" dominant-baseline="central" font-family="{MONO}" '
             ...
             f'COCOINDEX · MEETING NOTES → KNOWLEDGE GRAPH FLOW</text>')
    y += 30 + 28

    s.append(fnode(CX, y, NODE_W, "logic", "Source", "google_drive",
                   "Markdown meeting notes · shared folders"))
    y += FNODE_H
    s.append(vdown(CX, y)); y += VGAP

    s.append(fnode(CX, y, NODE_W, "data", "Data", "notes", "one DriveFile per note"))
    y += FNODE_H
    s.append(vdown(CX, y)); y += VGAP

    # ── Phase 1 ──
    s.append(fcomp(CX, y, COMP_W, COMP_H, "Phase 1 · Processing component",
                   "use_mount → process_file (per note, memoized)"))
    iy = y + COMP_PAD_T
    s.append(fnode(CX, iy, INNER_W, "logic", "Transform · split + LLM extract",
                   "instructor + LiteLLM",
                   "extract_meeting · date · organizer · participants · tasks"))
    iy += FNODE_H
    s.append(vdown(CX, iy)); iy += VGAP
    s.append(fnode(CX - ROW_W / 2 - 15, iy, ROW_W, "target", "Target state",
                   "Meeting · Task nodes", "declare_record · DECIDED edge"))
    s.append(fnode(CX + ROW_W / 2 + 15, iy, ROW_W, "data", "Data · carried forward",
                   "MeetingExtraction", "raw person names · task assignees"))
    y += COMP_H
    s.append(vdown(CX, y)); y += VGAP

    # ── Phase 2 ──
    s.append(fcomp(CX, y, COMP_W, COMP_H, "Phase 2 · Processing component",
                   "use_mount → resolve_persons (one pass, all notes)"))
    iy = y + COMP_PAD_T
    s.append(fnode(CX, iy, INNER_W, "logic", "Transform · entity resolution",
                   "embed + LLM resolve",
                   "resolve_entities · arctic-embed + LlmPairResolver"))
    iy += FNODE_H
    s.append(vdown(CX, iy)); iy += VGAP
    s.append(fnode(CX, iy, INNER_W, "data", "Data · carried forward", "ResolvedEntities",
                   '"Alice", "Alice Chen", "alice c." → one canonical name'))
    y += COMP_H
    s.append(vdown(CX, y)); y += VGAP

    # ── Phase 3 ──
    s.append(fcomp(CX, y, COMP_W, COMP_H, "Phase 3 · Processing component",
                   "mount → create_person_relations (one pass)"))
    iy = y + COMP_PAD_T
    s.append(fnode(CX, iy, INNER_W, "logic", "Transform", "aggregate · resolved names",
                   "organizer flag wins · dedup per person", mono=False))
    iy += FNODE_H
    s.append(vdown(CX, iy)); iy += VGAP
    s.append(fnode(CX, iy, INNER_W, "target", "Target states",
                   "Person nodes · ATTENDED · ASSIGNED_TO",
                   "declare_record · declare_relation"))
    y += COMP_H
    s.append(vdown(CX, y)); y += VGAP

    s.append(fnode(CX, y, NODE_W, "logic", "Target", "GraphDB",
                   "property graph · Person · Meeting · Task", mono=False))
    y += FNODE_H
...
DIAGRAMS = {
    "flow-v1": (flow_svg, 0),
    "stage-phase1": (phase1_svg, 34),
    "stage-phase2": (phase2_svg, 34),
    "stage-phase3": (phase3_svg, 34),
}
