...
Renders:
  flow-v1.png        full vertical pipeline (hero), all three phases
  stage-phase1.png   per-note extraction components highlighted
  stage-phase2.png   single person entity-resolution pass highlighted
  stage-phase3.png   single person-relations pass highlighted
...
.sw.comp{{background:color-mix(in oklab,{PEACH} 13%,{CREAM});border-color:{CORAL};border-radius:8px;}}
  ...
  <div class="eyebrow"><span class="dot"></span>CocoIndex &middot; Meeting notes &rarr; knowledge graph flow</div>

  <div class="col">
    {node("logic", "Source", "google_drive", "Markdown meeting notes &middot; shared folders")}
    {ARROW}
    {node("data", "Data", "notes", "one DriveFile per note")}
    {ARROW}

    <div class="comp">
      <div class="comp-cap">Phase 1 &middot; Processing Component &middot; <span class="reg">use_mount &rarr; process_file (per note, memoized)</span></div>
      <div class="col">
        {node("logic", "Transform &middot; split + LLM extract", "instructor + LiteLLM", "extract_meeting &middot; date &middot; organizer &middot; participants &middot; tasks")}
        {ARROW}
        <div class="row">
          {node("target", "Target state", "Meeting &middot; Task nodes", "declare_record &middot; DECIDED edge")}
          {node("data", "Data &middot; carried forward", "MeetingExtraction", "raw person names &middot; task assignees")}
        </div>
      </div>
    </div>
    ...
    <div class="comp">
      <div class="comp-cap">Phase 2 &middot; Processing Component &middot; <span class="reg">use_mount &rarr; resolve_persons (one pass, all notes)</span></div>
      <div class="col">
        {node("logic", "Transform &middot; entity resolution", "embed + LLM resolve", "resolve_entities &middot; arctic-embed + LlmPairResolver")}
        {ARROW}
        {node("data", "Data &middot; carried forward", "ResolvedEntities", '"Alice", "Alice Chen", "alice c." &rarr; one canonical name')}
      </div>
    </div>
    ...
    <div class="comp">
      <div class="comp-cap">Phase 3 &middot; Processing Component &middot; <span class="reg">mount &rarr; create_person_relations (one pass)</span></div>
      <div class="col">
        {node("logic", "Transform", "aggregate &middot; resolved names", "organizer flag wins &middot; dedup per person")}
        {ARROW}
        {node("target", "Target states", "Person nodes &middot; ATTENDED &middot; ASSIGNED_TO", "declare_record &middot; declare_relation")}
      </div>
    </div>

    {ARROW}
    {node("logic", "Target", "Neo4j", "property graph &middot; Person &middot; Meeting &middot; Task")}
  </div>
...
if "--render" in sys.argv:
    ...
    subprocess.run([
        ...
        f"--screenshot={OUT_DIR / 'flow-v1.png'}", "--window-size=1240,1980",
