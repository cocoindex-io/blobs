#!/usr/bin/env python3
"""V1 diagrams for the `face-recognition` example.

Adapted from the text-embedding / image-search generators, fitted to
examples/face_recognition/main.py:
  localfs.walk_dir(images, live) -> files -> [process_file (per image, memoized):
  extract_faces (dlib CNN, GPU runner) -> faces -> coco.map process_face:
  embed_face (128-d) -> declare_point] -> Qdrant collection (euclid).

The cover is reused from the v0 face-detection blog, not generated here.

Renders:
  flow-v1.png            full vertical pipeline (hero)
  stage-file-process.png per-image fan-out (mount_each)
...
.arrow .head{{width:0;height:0;border-left:7px solid transparent;border-right:7px solid transparent;border-top:9px solid {MAROON};}}
...
.sw.logic{{border-radius:13px;}} .sw.data{{border-radius:4px;}} .sw.target{{border-radius:3px 13px 13px 3px;}}
.sw.comp{{background:color-mix(in oklab,{PEACH} 13%,{CREAM});border-color:{CORAL};border-radius:8px;}}
</style></head><body>
  <div class="eyebrow"><span class="dot"></span>CocoIndex &middot; Face recognition flow (indexing)</div>
  <div class="col">
    {node("logic", "Source", "localfs.walk_dir", "Local filesystem · *.jpg / *.png · watches for changes (live=True)")}
    {ARROW}
    {node("data", "Data", "files", "one FileLike per image")}
    {ARROW}
    <div class="comp">
      <div class="comp-cap">Processing Component &middot; <span class="reg">mount_each &rarr; process_file (per image, memoized)</span></div>
      <div class="col">
        {node("logic", "Transform · dlib · GPU runner", "extract_faces", "detect faces (CNN) → one cropped PNG + box each")}
        {ARROW}
        {node("data", "Data", "faces", "Face · bounding box + crop")}
        {ARROW}
        <div class="subcomp">
          <div class="subcomp-cap">coco.map &rarr; process_face (per face)</div>
          <div class="col">
            {node("logic", "Transform · dlib · GPU runner", "embed_face", "face → 128-d embedding")}
            {ARROW}
            {node("target", "Target state · one per face", "declare_point → Qdrant", "id (uuid5 of file + box) · vector · payload {filename, box}")}
          </div>
        </div>
      </div>
    </div>
    {ARROW}
    {node("logic", "Target", "Qdrant", "face_embeddings collection · euclidean")}
  </div>
  <div class="legend">
...
# ─────────────────────────── stage (SVG) ───────────────────────────
def _stroke(base, hl): return (PALM, 4.0) if hl else base
def tlines(cx, cy, lines, size, weight=600, color=MAROON_INK, mono=False):
    ...
    lh = size * 1.16; y0 = cy - lh * (len(lines) - 1) / 2
    spans = "".join(f'<tspan x="{cx}" y="{y0 + i*lh:.1f}">{t}</tspan>' for i, t in enumerate(lines))
    return (f'<text text-anchor="middle" dominant-baseline="central" font-family="{fam}" '
...
def logic(x, y, w, h, lines, *, hl=False, size=26, mono=False, sub=None):
    ...
    body = f'<rect x="{x}" y="{y}" width="{w}" height="{h}" rx="18" fill="{CREAM}" stroke="{st}" stroke-width="{sw}"/>'
    if sub:
...
def data(x, y, w, h, lines, *, hl=False, size=23, mono=True):
    st, sw = _stroke((MAROON, 2.4), hl)
    return (f'<rect x="{x}" y="{y}" width="{w}" height="{h}" rx="5" fill="{CREAM}" stroke="{st}" stroke-width="{sw}"/>'
            + tlines(x + w/2, y + h/2, lines, size, mono=mono))
def bullet(x, y, w, h, lines, *, hl=False, size=21):
    st, sw = _stroke((MAROON, 2.4), hl); r = h / 2
...
def app(x, y, w, h, label, *, hl=False):
    ...
    return (f'<rect x="{x}" y="{y}" width="{w}" height="{h}" rx="18" fill="{APP_FILL}" stroke="{st}" stroke-width="{sw}"/>'
            + tlines(x + w/2, y + 26, [label], 19, weight=600, color=CORAL, mono=True))
def pc(x, y, w, h, label, *, hl=False):
    st, sw = _stroke((MAROON, 2.2), hl)
    return (f'<rect x="{x}" y="{y}" width="{w}" height="{h}" rx="11" fill="{CREAM}" stroke="{st}" stroke-width="{sw}"/>'
            + tlines(x + w/2, y + 24, [label], 18, weight=600, color=MUTED, mono=True))
def arrow(x1, y, x2):
    return f'<line x1="{x1}" y1="{y}" x2="{x2-11}" y2="{y}" stroke="{MAROON}" stroke-width="2.6" marker-end="url(#ah)"/>'
def bind(x1, y, x2):
    return f'<line x1="{x1}" y1="{y}" x2="{x2}" y2="{y}" stroke="{MAROON}" stroke-width="2.4" stroke-dasharray="7 6"/>'
DEFS = (f'<defs><marker id="ah" markerWidth="13" markerHeight="12" refX="10" refY="4" orient="auto" '
        f'markerUnits="userSpaceOnUse"><path d="M0 0 L11 4 L0 8 Z" fill="{MAROON}"/></marker></defs>')

def fanout_svg():
    APP_X = 250; APP_Y = 40; APP_H = 464
    PADX = 30; FILE_W = 150; FILE_H = 52; GAPF = 40
    PC_PADX = 28; XFM_W = 330; XFM_H = 92; GAPX = 36; OUT_W = 210; OUT_H = 56
    PCW = PC_PADX + XFM_W + GAPX + OUT_W + PC_PADX; PCH = 152
    APP_W = PADX + FILE_W + GAPF + PCW + PADX
    TGT_W = 196; TGT_X = APP_X + APP_W + 40
    W = TGT_X + TGT_W + 40; H = APP_Y + APP_H + 32
    file_x = APP_X + PADX; pc_x = file_x + FILE_W + GAPF; xfm_x = pc_x + PC_PADX; out_x = xfm_x + XFM_W + GAPX
    SRC = (40, APP_Y, 178, APP_H)
    s = [DEFS]
    s.append(logic(*SRC, ["Images", "folder"], size=26, sub="walk_dir · live"))
    s.append(app(APP_X, APP_Y, APP_W, APP_H, "CocoIndex App"))
    s.append(logic(TGT_X, APP_Y, TGT_W, APP_H, ["Qdrant"], size=26, sub="collection"))
    for cy, fname in [(160, "solvay.jpg"), (340, "einplanck.jpg")]:
        s.append(bind(SRC[0] + SRC[2], cy, file_x))
        s.append(data(file_x, cy - FILE_H/2, FILE_W, FILE_H, [fname], hl=True, size=19))
        s.append(arrow(file_x + FILE_W, cy, pc_x))
        s.append(pc(pc_x, cy - PCH/2, PCW, PCH, "process_file", hl=True))
        s.append(logic(xfm_x, cy - XFM_H/2 + 10, XFM_W, XFM_H, ["detect + embed faces"], hl=True, size=22))
        s.append(arrow(xfm_x + XFM_W, cy, out_x))
        s.append(bullet(out_x, cy - OUT_H/2, OUT_W, OUT_H, ["face points (×N)"], hl=True, size=18))
...
def page(w, h, body):
    return (f'<!doctype html><html><head><meta charset="utf-8">'
            ...
            f'<link href="{GF}" rel="stylesheet"><style>*{{margin:0;padding:0;box-sizing:border-box}}'
            f'body{{background:{CREAM};padding:34px}}svg{{display:block}}</style></head><body>'
            f'<svg xmlns="http://www.w3.org/2000/svg" width="{w}" height="{h}" viewBox="0 0 {w} {h}">{body}</svg></body></html>')

(OUT_DIR / "flow.html").write_text(flow_html()); print("wrote flow.html")
w, h, body = fanout_svg()
(OUT_DIR / "stage-file-process.html").write_text(page(w, h, body)); print("wrote stage-file-process.html")

if "--render" in sys.argv:
    chrome = "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome"
    subprocess.run([chrome, "--headless=new", "--hide-scrollbars", "--no-sandbox", "--force-color-profile=srgb",
        "--virtual-time-budget=8000", f"--screenshot={OUT_DIR / 'flow-v1.png'}", "--window-size=1240,1400",
        (OUT_DIR / "flow.html").as_uri()], check=True); print("rendered flow-v1.png")
    subprocess.run([chrome, "--headless=new", "--hide-scrollbars", "--no-sandbox", "--force-color-profile=srgb",
        "--virtual-time-budget=8000", f"--screenshot={OUT_DIR / 'stage-file-process.png'}",
        "--window-size={},{}".format(w + 68, h + 68), (OUT_DIR / "stage-file-process.html").as_uri()], check=True)
    print("rendered stage-file-process.png")
