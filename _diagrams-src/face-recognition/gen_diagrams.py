...
Renders (SVG is what the docs page embeds; HTML files are local previews):
  ...
  stage-file-process.svg     per-image fan-out (mount_each)
...
# peach = component, dashed = per-face sub-component).
...
def flow_svg():
    ...
    s.append(f'<text x="82" y="{y+11}" dominant-baseline="central" font-family="{MONO}" '
             ...
             f'COCOINDEX · FACE RECOGNITION FLOW (INDEXING)</text>')
    y += 30 + 28
    ...
    iy = y + COMP_PAD_T
    s.append(fnode(CX, iy, INNER_W, "logic", "Transform · dlib · GPU runner",
                   "extract_faces", "detect faces (CNN) → one cropped PNG + box each"))
    iy += FNODE_H
    s.append(vdown(CX, iy)); iy += VGAP
    s.append(fnode(CX, iy, INNER_W, "data", "Data", "faces", "Face · bounding box + crop"))
    iy += FNODE_H
    s.append(vdown(CX, iy)); iy += VGAP

    s.append(fsubcomp(CX, iy, SUB_W, SUB_H, "coco.map → process_face (per face)"))
    sy = iy + SUB_PAD_T
    s.append(fnode(CX, sy, SUB_INNER_W, "logic", "Transform · dlib · GPU runner",
                   "embed_face", "face → 128-d embedding"))
    sy += FNODE_H
    s.append(vdown(CX, sy)); sy += VGAP
    s.append(fnode(CX, sy, SUB_INNER_W, "target", "Target state · one per face",
                   "declare_point → Qdrant",
                   "id (uuid5 of file + box) · vector · payload {filename, box}"))
    y += COMP_H
    s.append(vdown(CX, y)); y += VGAP

    s.append(fnode(CX, y, NODE_W, "logic", "Target", "Qdrant",
                   "face_embeddings collection · euclidean"))
    y += FNODE_H
