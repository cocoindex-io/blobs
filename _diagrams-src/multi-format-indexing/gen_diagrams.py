#!/usr/bin/env python3
"""V1 diagrams for the `multi-format-indexing` example (cover + flow-v1)."""
...
</style></head><body><div class="eyebrow"><span class="dot"></span>CocoIndex &middot; Multi-format indexing flow</div><div class="col">
{node("logic","Source","localfs.walk_dir","source_files · *.pdf / *.jpg / *.png")}{ARROW}
<div class="comp"><div class="comp-cap">Processing Component &middot; <span class="reg">mount_each &rarr; process_file (per file, memoized)</span></div><div class="col">
{node("logic","Transform · pdf2image","file_to_pages","PDF → one image per page · image → one page")}{ARROW}
{node("data","Data","pages","Page · page_number + image")}{ARROW}
<div class="comp" style="background:#F6F4E9;border:1.8px dashed color-mix(in oklab,#BE5133 60%,transparent)"><div class="comp-cap" style="color:#532638aa">coco.map &rarr; process_page (per page)</div><div class="col">
{node("logic","Transform · ColPali","embed_page","page image → multi-vector embedding")}{ARROW}
{node("target","Target state · one per page","declare_point → Qdrant","id · vectors · payload {filename, page}")}</div></div></div></div>{ARROW}
{node("logic","Target","Qdrant","collection · multi-vector · MaxSim")}
