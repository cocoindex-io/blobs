#!/usr/bin/env python3
"""V1 diagrams for the `hackernews-trending-topics` example.

Adapted from _diagrams-src/pdf-embedding/gen_diagrams.py, fitted to
examples/hn_trending_topics/main.py:
  fetch_thread_list (Algolia HN API) -> thread_ids -> [process_thread
  (per thread): fetch_thread (HN API) -> extract_topics (LLM, per message)
  -> declare_row(HnMessage) + declare_row(HnTopic)] -> Postgres
  (two tables: hn_messages, hn_topics).
...
Renders (SVG is what the docs page embeds; HTML files are local previews):
  ...
  stage-file-process.svg   per-thread transform chain highlighted
  stage-main-function.svg  thread fan-out + process_thread highlighted
...
def cover_html():
    ...
    pipeline = sep.join([chip("HackerNews"), chip("Threads"), chip("Comments"),
                         chip("Topics"), chip("Postgres")])
...
.coco{{position:absolute;top:60px;right:78px;font-size:60px;}}
  ...
  <div>
    <h1>Trending Topics<br>from <em>HackerNews</em></h1>
    <div class="sub">Scrape recent HN threads and comments via the Algolia API, extract topics with an LLM, and rank what's trending in Postgres — incremental, in plain async Python.</div>
  </div>
...
def fanout_svg(*, hl_fn=False, hl_file=False, hl_pc=False):
    ...
    s = [DEFS]
    s.append(logic(*SRC, ["Algolia", "HN API"], size=26, sub="thread ids"))
    s.append(app(APP_X, APP_Y, APP_W, APP_H, "CocoIndex App"))
    s.append(logic(TGT_X, APP_Y, TGT_W, APP_H, ["Postgres"], size=26, sub="2 tables"))

    for cy, fname in [(160, "thread 4521…"), (340, "thread 4519…")]:
        s.append(bind(SRC[0] + SRC[2], cy, file_x))
        s.append(data(file_x, cy - FILE_H/2, FILE_W, FILE_H, [fname], hl=hl_file, size=17))
        s.append(arrow(file_x + FILE_W, cy, pc_x))
        s.append(pc(pc_x, cy - PCH/2, PCW, PCH, "process_thread", hl=hl_pc))
        s.append(logic(xfm_x, cy - XFM_H/2 + 10, XFM_W, XFM_H,
                       ["fetch · extract topics (LLM)"], hl=hl_fn, size=21))
        s.append(arrow(xfm_x + XFM_W, cy, out_x))
        s.append(bullet(out_x, cy - OUT_H/2, OUT_W, OUT_H,
                        ["HnMessage +", "HnTopic rows"], hl=hl_fn, size=17))
...
# peach = component, dashed = per-message sub-component).
...
def flow_svg():
    ...
    s.append(f'<text x="82" y="{y+11}" dominant-baseline="central" font-family="{MONO}" '
             ...
             f'COCOINDEX · HACKERNEWS TRENDING TOPICS FLOW</text>')
    y += 30 + 28

    s.append(fnode(CX, y, NODE_W, "logic", "Source · custom", "Algolia HN API",
                   "fetch_thread_list · recent stories (one-shot catch-up)"))
    y += FNODE_H
    s.append(vdown(CX, y)); y += VGAP

    s.append(fnode(CX, y, NODE_W, "data", "Data", "thread_ids",
                   "one string id per HN story"))
    y += FNODE_H
    s.append(vdown(CX, y)); y += VGAP

    # ── processing component ──
    s.append(fcomp(CX, y, COMP_W, COMP_H, "Processing component",
                   "mount_each → process_thread (per thread)"))
    iy = y + COMP_PAD_T
    s.append(fnode(CX, iy, INNER_W, "logic", "Transform · HN API", "fetch_thread",
                   "thread id → Thread (title, text, comments)"))
    iy += FNODE_H
    s.append(vdown(CX, iy)); iy += VGAP
    s.append(fnode(CX, iy, INNER_W, "data", "Data", "thread + comments",
                   "one message per thread &amp; per comment"))
    iy += FNODE_H
    s.append(vdown(CX, iy)); iy += VGAP

    s.append(fsubcomp(CX, iy, SUB_W, SUB_H, "per message"))
    sy = iy + SUB_PAD_T
    s.append(fnode(CX, sy, SUB_INNER_W, "logic", "Transform · LLM", "extract_topics",
                   "message text → list[str] topics (litellm)", hl=True))
    sy += FNODE_H
    s.append(vdown(CX, sy)); sy += VGAP
    s.append(fnode(CX, sy, SUB_INNER_W, "target", "Target state",
                   "declare_row → HnMessage + HnTopic",
                   "messages · (topic, message_id) rows"))
    y += COMP_H
    s.append(vdown(CX, y)); y += VGAP

    s.append(fnode(CX, y, NODE_W, "logic", "Target", "Postgres",
                   "coco_examples.hn_messages · hn_topics"))
    y += FNODE_H
