...
Renders:
  ...
  stage-file-process.png per-thread transform chain highlighted
...
.sw.comp{{background:color-mix(in oklab,{PEACH} 13%,{CREAM});border-color:{CORAL};border-radius:8px;}}
  ...
  <div class="eyebrow"><span class="dot"></span>CocoIndex &middot; HackerNews trending topics flow</div>

  <div class="col">
    {node("logic", "Source &middot; custom", "Algolia HN API", "fetch_thread_list &middot; recent stories (one-shot catch-up)")}
    {ARROW}
    {node("data", "Data", "thread_ids", "one string id per HN story")}
    {ARROW}

    <div class="comp">
      <div class="comp-cap">Processing Component &middot; <span class="reg">mount_each &rarr; process_thread (per thread)</span></div>
      <div class="col">
        {node("logic", "Transform &middot; HN API", "fetch_thread", "thread id &rarr; Thread (title, text, comments)")}
        {ARROW}
        {node("data", "Data", "thread + comments", "one message per thread &amp; per comment")}
        {ARROW}
        <div class="subcomp">
          <div class="subcomp-cap">per message</div>
          <div class="col">
            {node("logic", "Transform &middot; LLM", "extract_topics", "message text &rarr; list[str] topics (litellm)", hl=True)}
            {ARROW}
            {node("target", "Target state", "declare_row &rarr; HnMessage + HnTopic", "messages &middot; (topic, message_id) rows")}
          </div>
        </div>
      </div>
    </div>
    ...
    {ARROW}
    {node("logic", "Target", "Postgres", "coco_examples.hn_messages &middot; hn_topics")}
  </div>
...
if "--render" in sys.argv:
    ...
    subprocess.run([
        chrome, "--headless=new", "--hide-scrollbars", "--no-sandbox",
        "--force-color-profile=srgb", "--virtual-time-budget=8000",
        f"--screenshot={OUT_DIR / 'flow-v1.png'}", "--window-size=1240,1760",
