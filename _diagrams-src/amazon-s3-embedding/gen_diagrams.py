...
(Amazon S3 instead of the local filesystem). Renders only cover + flow.
...
def cover_html():
    ...
    pipeline = sep.join([chip("S3 bucket"), chip("Chunks"),
                         chip("Vectors"), chip("pgvector")])
...
.coco{{position:absolute;top:60px;right:78px;font-size:60px;}}
  ...
  <div>
    <h1>Embed Markdown<br>from <em>Amazon S3</em></h1>
    <div class="sub">List Markdown objects from an S3 bucket, embed each chunk, and store the vectors in Postgres with pgvector — incremental, in plain async Python.</div>
  </div>
...
.sw.comp{{background:color-mix(in oklab,{PEACH} 13%,{CREAM});border-color:{CORAL};border-radius:8px;}}
  ...
  <div class="eyebrow"><span class="dot"></span>CocoIndex &middot; Text embedding flow &middot; Amazon S3</div>

  <div class="col">
    {node("logic", "Source", "amazon_s3.list_objects", "S3 bucket &middot; prefix + *.md glob &middot; catch-up")}
    {ARROW}
    {node("data", "Data", "files", "one S3File per matching object")}
    {ARROW}
    ...
    {ARROW}
    {node("logic", "Target", "Postgres &middot; pgvector", "amazon_s3_doc_embeddings table")}
  </div>
