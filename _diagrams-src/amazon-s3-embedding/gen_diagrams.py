...
def flow_svg():
    ...
    s.append(f'<text x="82" y="{y+11}" dominant-baseline="central" font-family="{MONO}" '
             ...
             f'COCOINDEX · TEXT EMBEDDING FLOW · AMAZON S3</text>')
    y += 30 + 28

    s.append(fnode(CX, y, NODE_W, "logic", "Source", "amazon_s3.list_objects",
                   "S3 bucket · prefix + *.md glob · catch-up"))
    y += FNODE_H
    s.append(vdown(CX, y)); y += VGAP

    s.append(fnode(CX, y, NODE_W, "data", "Data", "files", "one S3File per matching object"))
    y += FNODE_H
    s.append(vdown(CX, y)); y += VGAP
    ...
    s.append(fnode(CX, y, NODE_W, "logic", "Target", "Postgres · pgvector",
                   "amazon_s3_doc_embeddings table", mono=False))
    y += FNODE_H
