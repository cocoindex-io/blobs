#!/usr/bin/env python3
"""V1 diagrams for the `slides-to-speech` example.

Fitted to examples/slides_to_speech/main.py:
  walk_dir(*.pdf) -> [process_file (per deck, memoized): pymupdf
  pdf_to_slides -> slides -> coco.map process_slide: vision LLM
  extract_speaker_notes -> Piper TTS + embed -> declare_row SlideRecord]
  -> LanceDB slides_to_speech table.
...
# peach = component, dashed = per-slide sub-component).
...
def flow_svg():
    ...
    SUB_ROW_W = (SUB_INNER_W - 30) / 2

    SUB_H = SUB_PAD_T + FNODE_H + VGAP + FNODE_H + VGAP + FNODE_H + SUB_PAD_B
    ...
    s.append(f'<text x="82" y="{y+11}" dominant-baseline="central" font-family="{MONO}" '
             ...
             f'COCOINDEX · SLIDES TO SPEECH FLOW</text>')
    y += 30 + 28
    ...
    s.append(fcomp(CX, y, COMP_W, COMP_H, "Processing component",
                   "mount_each → process_file (per deck, memoized)"))
    iy = y + COMP_PAD_T
    s.append(fnode(CX, iy, INNER_W, "logic", "Transform · pymupdf", "pdf_to_slides",
                   "PDF → one image per slide"))
    iy += FNODE_H
    s.append(vdown(CX, iy)); iy += VGAP
    s.append(fnode(CX, iy, INNER_W, "data", "Data", "slides", "SlidePage · page + image"))
    iy += FNODE_H
    s.append(vdown(CX, iy)); iy += VGAP

    s.append(fsubcomp(CX, iy, SUB_W, SUB_H, "coco.map → process_slide (per slide)"))
    sy = iy + SUB_PAD_T
    s.append(fnode(CX, sy, SUB_INNER_W, "logic", "Transform · vision LLM",
                   "extract_speaker_notes",
                   "slide image → speaker notes"))
    sy += FNODE_H
    s.append(vdown(CX, sy)); sy += VGAP
    s.append(fnode(CX - SUB_ROW_W / 2 - 15, sy, SUB_ROW_W, "logic",
                   "Transform · Piper TTS", "text_to_speech", "notes → MP3 audio"))
    s.append(fnode(CX + SUB_ROW_W / 2 + 15, sy, SUB_ROW_W, "logic",
                   "Transform · embed", "SentenceTransformer", "notes → vector"))
    sy += FNODE_H
    s.append(vdown(CX, sy)); sy += VGAP
    s.append(fnode(CX, sy, SUB_INNER_W, "target", "Target state · one per slide",
                   "declare_row → SlideRecord",
                   "page · notes · voice · embedding"))
    y += COMP_H
    s.append(vdown(CX, y)); y += VGAP

    s.append(fnode(CX, y, NODE_W, "logic", "Target", "LanceDB",
                   "slides_to_speech table", mono=False))
    y += FNODE_H
