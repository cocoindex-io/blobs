...
Adapted from the index-codebase-v1 blog generators (gen_flow.py, gen_stages.py),
fitted to examples/text_embedding/main.py:
...
Renders:
  ...
  stage-file-process.png per-file transform + output highlighted
  stage-main-function.png files + processing components highlighted (mount_each)
  stage-create-app.png   App container highlighted (binding the app)
...
.sw.comp{{background:color-mix(in oklab,{PEACH} 13%,{CREAM});border-color:{CORAL};border-radius:8px;}}
  ...
  <div class="eyebrow"><span class="dot"></span>CocoIndex &middot; Text embedding flow</div>

  <div class="col">
    {node("logic", "Source", "localfs.walk_dir", "Local filesystem &middot; watches for changes (live=True)")}
    {ARROW}
    ...
    <div class="comp">
      ...
      <div class="col">
        {node("logic", "Transform &middot; Markdown", "RecursiveSplitter", "split into overlapping chunks (2000 / 500)", hl=True)}
        {ARROW}
    ...
    {ARROW}
    {node("logic", "Target", "Postgres &middot; pgvector", "doc_embeddings table &middot; vector index (cosine)")}
  </div>
...
if "--render" in sys.argv:
    ...
    subprocess.run([
        ...
        f"--screenshot={OUT_DIR / 'flow-v1.png'}", "--window-size=1240,1620",
