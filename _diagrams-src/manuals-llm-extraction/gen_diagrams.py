#!/usr/bin/env python3
"""V1 diagrams for the `manuals-llm-extraction` example.

Fitted to examples/manuals_llm_extraction/main.py:
  walk_dir(*.pdf) -> [process_file (per manual, memoized): docling
  pdf_to_markdown -> LLM extract ModuleInfo -> declare_row ModuleRecord]
  -> Postgres modules_info table.
...
def cover_html():
    ...
    pipe=sep.join([chip("PDF manuals"),chip("docling"),chip("LLM"),chip("Postgres")])
...
<div><h1>Manuals to<br><em>Structured Data</em></h1><div class="sub">Convert PDF manuals to Markdown with docling, LLM-extract a typed module summary, and store it in Postgres.</div></div>
...
def flow_svg():
    ...
    s.append(f'<text x="82" y="{y+11}" dominant-baseline="central" font-family="{MONO}" '
             ...
             f'COCOINDEX · MANUALS EXTRACTION FLOW</text>')
    y += 30 + 28
    ...
    s.append(fcomp(CX, y, COMP_W, COMP_H, "Processing component",
                   "mount_each → process_file (per manual, memoized)"))
    iy = y + COMP_PAD_T
    s.append(fnode(CX, iy, INNER_W, "logic", "Transform · docling · GPU runner",
                   "pdf_to_markdown", "PDF bytes → Markdown"))
    iy += FNODE_H
    s.append(vdown(CX, iy)); iy += VGAP
    s.append(fnode(CX, iy, INNER_W, "logic", "Transform · LLM extract",
                   "instructor + LiteLLM",
                   "ModuleInfo · title · classes · methods · args"))
    iy += FNODE_H
    s.append(vdown(CX, iy)); iy += VGAP
    s.append(fnode(CX, iy, INNER_W, "target", "Target state · one per manual",
                   "declare_row → ModuleRecord",
                   "filename · title · num_classes · num_methods · module_info"))
    y += COMP_H
    s.append(vdown(CX, y)); y += VGAP

    s.append(fnode(CX, y, NODE_W, "logic", "Target", "Postgres",
                   "modules_info table", mono=False))
    y += FNODE_H
