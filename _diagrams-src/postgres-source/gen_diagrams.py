#!/usr/bin/env python3
"""V1 diagrams for the `postgres-source` example.

Adapted from _diagrams-src/pdf-embedding/gen_diagrams.py, fitted to
examples/postgres_source/main.py:
  PgTableSource.fetch_rows(source_products) -> rows ->
  [process_product (per row, memoized): derive full_description + total_value
  -> embed -> declare_row(OutputProduct)] -> Postgres (pgvector).

Renders:
  cover.png              social/hero cover
  flow-v1.png            full vertical pipeline (hero)
  stage-file-process.png per-row transform chain highlighted
...
def cover_html():
    ...
    pipeline = sep.join([chip("Postgres table"), chip("Rows"), chip("Derive"),
...
.coco{{position:absolute;top:60px;right:78px;font-size:60px;}}
  ...
  <div>
    <h1>Postgres as a<br><em>source</em></h1>
    <div class="sub">Read rows from an existing Postgres table, derive fields, embed, and write the vectors back to Postgres — incremental, in plain async Python.</div>
  </div>
...
.sw.comp{{background:color-mix(in oklab,{PEACH} 13%,{CREAM});border-color:{CORAL};border-radius:8px;}}
  ...
  <div class="eyebrow"><span class="dot"></span>CocoIndex &middot; Postgres source flow</div>

  <div class="col">
    {node("logic", "Source", "postgres.PgTableSource", "source_products table &middot; fetch_rows() (cursor stream)")}
    {ARROW}
    {node("data", "Data", "rows", "one SourceProduct per row &middot; key (category, name)")}
    {ARROW}

    <div class="comp">
      <div class="comp-cap">Processing Component &middot; <span class="reg">mount_each &rarr; process_product (per row, memoized)</span></div>
      <div class="col">
        {node("logic", "Transform", "derive fields", "full_description (category + name + body) &middot; total_value = price &times; amount", hl=True)}
        {ARROW}
        {node("logic", "Transform", "SentenceTransformerEmbedder.embed", "full_description &rarr; embedding (384-d vector)", hl=True)}
        {ARROW}
        {node("target", "Target state", "declare_row &rarr; OutputProduct", "category &middot; name &middot; price &middot; amount &middot; total_value &middot; embedding")}
      </div>
    </div>

    {ARROW}
    {node("logic", "Target", "Postgres &middot; pgvector", "coco_examples_v1.output table &middot; sequential scan")}
  </div>
...
def fanout_svg(*, hl_fn=False, hl_file=False, hl_pc=False):
    ...
    PC_PADX = 28; XFM_W = 320; XFM_H = 92; GAPX = 36; OUT_W = 200; OUT_H = 56
