...
Renders (SVG is what the docs page embeds; HTML files are local previews):
  ...
  stage-file-process.svg  per-row transform chain highlighted
...
def flow_svg():
    ...
    COMP_H = COMP_PAD_T + FNODE_H + VGAP + FNODE_H + VGAP + FNODE_H + COMP_PAD_B
    ...
    s.append(f'<text x="82" y="{y+11}" dominant-baseline="central" font-family="{MONO}" '
             ...
             f'COCOINDEX · POSTGRES SOURCE FLOW</text>')
    y += 30 + 28

    s.append(fnode(CX, y, NODE_W, "logic", "Source", "postgres.PgTableSource",
                   "source_products table · fetch_rows() (cursor stream)"))
    y += FNODE_H
    s.append(vdown(CX, y)); y += VGAP

    s.append(fnode(CX, y, NODE_W, "data", "Data", "rows",
                   "one SourceProduct per row · key (category, name)"))
    y += FNODE_H
    s.append(vdown(CX, y)); y += VGAP

    # ── processing component ──
    s.append(fcomp(CX, y, COMP_W, COMP_H, "Processing component",
                   "mount_each → process_product (per row, memoized)"))
    iy = y + COMP_PAD_T
    s.append(fnode(CX, iy, INNER_W, "logic", "Transform", "derive fields",
                   "full_description (category + name + body) · total_value = price × amount",
                   hl=True))
    iy += FNODE_H
    s.append(vdown(CX, iy)); iy += VGAP
    s.append(fnode(CX, iy, INNER_W, "logic", "Transform",
                   "SentenceTransformerEmbedder.embed",
                   "full_description → embedding (384-d vector)", hl=True))
    iy += FNODE_H
    s.append(vdown(CX, iy)); iy += VGAP
    s.append(fnode(CX, iy, INNER_W, "target", "Target state",
                   "declare_row → OutputProduct",
                   "category · name · price · amount · total_value · embedding"))
    y += COMP_H
    s.append(vdown(CX, y)); y += VGAP

    s.append(fnode(CX, y, NODE_W, "logic", "Target", "Postgres · pgvector",
                   "coco_examples_v1.output table · sequential scan"))
    y += FNODE_H
