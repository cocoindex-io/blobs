#!/usr/bin/env python3
"""V1 diagrams for the `csv-to-kafka` example.

Adapted from _diagrams-src/meeting-notes-to-knowledge-graph/gen_diagrams.py,
fitted to examples/csv_to_kafka/main.py:

  localfs.walk_dir(live=True) -> files -> [process_csv (per file, memoized):
...
# ─────────────────── stage: per-file fan-out (mount_each) ───────────────────
def stage_svg():
    APP_X = 270; APP_Y = 40; APP_H = 530
    PADX = 30; FILE_W = 168; FILE_H = 52; GAPF = 40
    PC_PADX = 26; XFM_W = 300; XFM_H = 92; GAPX = 36; OUT_W = 290; PCH = 184
    ...
    SRC = (40, APP_Y, 192, APP_H)

    s = [DEFS]
    s.append(logic(*SRC, ["CSV files", "./data"], size=25, sub="walk_dir · live"))
    s.append(app(APP_X, APP_Y, APP_W, APP_H, "CocoIndex App"))
    s.append(logic(TGT_X, APP_Y, TGT_W, APP_H, ["Kafka", "topic"], size=25, sub="one topic"))

    for cy, fname in [(182, "products.csv"), (372, "employees.csv")]:
        s.append(bind(SRC[0] + SRC[2], cy, file_x))
        s.append(data(file_x, cy - FILE_H/2, FILE_W, FILE_H, [fname], hl=True, size=20))
        s.append(arrow(file_x + FILE_W, cy, pc_x))
        s.append(pc(pc_x, cy - PCH/2, PCW, PCH, "process_csv", hl=True))
        s.append(logic(xfm_x, cy - XFM_H/2 + 12, XFM_W, XFM_H,
                       ["row → JSON"], size=24, sub="DictReader · memoized"))
        s.append(arrow(xfm_x + XFM_W, cy, out_x))
        s.append(bullet(out_x, cy - 27, OUT_W, 54, ["(key, value) per row"], size=18))
