#!/usr/bin/env python3
"""V1 diagrams for the `patient-intake-baml` example.
...
examples/patient_intake_extraction_baml/main.py:
  ...
  extract_patient_info (BAML, Gemini vision) -> Patient (typed)
  -> declare_file(<stem>.json)] -> local JSON directory (output_patients/).
...
Renders (SVG is what the docs page embeds; HTML files are local previews):
  ...
  stage-file-process.svg per-file transform chain highlighted

Everything in the diagrams is vector: text stays live text with a web-safe
font stack (Inter / JetBrains Mono with system fallbacks), matching the
card.svg convention in this repo. No @font-face is embedded.
...
def cover_html():
    ...
    pipeline = sep.join([chip("Intake PDF"), chip("BAML"), chip("Patient"),
                         chip("JSON")])
...
.coco{{position:absolute;top:60px;right:78px;font-size:60px;}}
  ...
  <div>
    <h1>Patient Intake Forms<br>to <em>Typed</em> JSON</h1>
    <div class="sub">Extract structured patient records from intake PDFs with BAML — type-safe, schema-driven, incremental, in plain async Python.</div>
  </div>
...
def fanout_svg(*, hl_fn=False, hl_file=False, hl_pc=False):
    ...
    PADX = 30; FILE_W = 188; FILE_H = 52; GAPF = 40
    PC_PADX = 28; XFM_W = 330; XFM_H = 92; GAPX = 36; OUT_W = 188; OUT_H = 56
    ...
    s = [DEFS]
    s.append(logic(*SRC, ["Intake", "PDFs"], size=26, sub="walk_dir"))
    s.append(app(APP_X, APP_Y, APP_W, APP_H, "CocoIndex App"))
    s.append(logic(TGT_X, APP_Y, TGT_W, APP_H, ["JSON", "files"], size=26, sub="output_patients/"))

    for cy, fname in [(160, "David.pdf"), (340, "Emily.pdf")]:
...
def flow_svg():
    ...
    # Component height: caption block + four stacked nodes + bottom pad.
    COMP_H = COMP_PAD_T + 4 * FNODE_H + 3 * VGAP + COMP_PAD_B
    ...
    s.append(f'<text x="82" y="{y+11}" dominant-baseline="central" font-family="{MONO}" '
             ...
             f'COCOINDEX · PATIENT INTAKE EXTRACTION FLOW</text>')
    y += 30 + 28

    s.append(fnode(CX, y, NODE_W, "logic", "Source", "localfs.walk_dir",
                   "Local filesystem · *.pdf intake forms"))
    y += FNODE_H
    s.append(vdown(CX, y)); y += VGAP

    s.append(fnode(CX, y, NODE_W, "data", "Data", "files", "one FileLike per intake PDF"))
    y += FNODE_H
    s.append(vdown(CX, y)); y += VGAP
    ...
    iy = y + COMP_PAD_T
    s.append(fnode(CX, iy, INNER_W, "logic", "Transform · BAML · Gemini vision",
                   "extract_patient_info",
                   "PDF bytes → Patient · typed by patient.baml schema", hl=True))
    iy += FNODE_H
    s.append(vdown(CX, iy)); iy += VGAP
    s.append(fnode(CX, iy, INNER_W, "data", "Data", "Patient",
                   "typed model · demographics, insurance, meds, allergies …"))
    iy += FNODE_H
    s.append(vdown(CX, iy)); iy += VGAP
    s.append(fnode(CX, iy, INNER_W, "logic", "Transform", "model_dump_json",
                   "Patient → JSON string (indent=2)"))
    iy += FNODE_H
    s.append(vdown(CX, iy)); iy += VGAP
    s.append(fnode(CX, iy, INNER_W, "target", "Target state",
                   "declare_file → &lt;stem&gt;.json",
                   "one JSON file per intake form"))
    y += COMP_H
    s.append(vdown(CX, y)); y += VGAP

    s.append(fnode(CX, y, NODE_W, "logic", "Target", "Local filesystem",
                   "output_patients/ · structured JSON records"))
    y += FNODE_H
