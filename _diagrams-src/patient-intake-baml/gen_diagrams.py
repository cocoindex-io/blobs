...
.sw.comp{{background:color-mix(in oklab,{PEACH} 13%,{CREAM});border-color:{CORAL};border-radius:8px;}}
  ...
  <div class="col">
    {node("logic", "Source", "localfs.walk_dir", "Local filesystem &middot; *.pdf intake forms")}
    {ARROW}
    ...
    <div class="comp">
      ...
      <div class="col">
        {node("logic", "Transform &middot; BAML &middot; Gemini vision", "extract_patient_info", "PDF bytes &rarr; Patient &middot; typed by patient.baml schema", hl=True)}
        {ARROW}
        {node("data", "Data", "Patient", "typed model &middot; demographics, insurance, meds, allergies …")}
        {ARROW}
        {node("logic", "Transform", "model_dump_json", "Patient &rarr; JSON string (indent=2)")}
        {ARROW}
        {node("target", "Target state", "declare_file &rarr; &lt;stem&gt;.json", "one JSON file per intake form")}
      </div>
    </div>

    {ARROW}
    {node("logic", "Target", "Local filesystem", "output_patients/ &middot; structured JSON records")}
  </div>
