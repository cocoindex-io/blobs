...
</style></head><body><div class="eyebrow"><span class="dot"></span>CocoIndex &middot; Product recommendation flow</div><div class="col">
{node("logic","Source","localfs.walk_dir","product JSON files")}{ARROW}
<div class="comp"><div class="comp-cap">Phase 1 &middot; Processing Component &middot; <span class="reg">use_mount &rarr; process_file (per product)</span></div><div class="col">
{node("logic","Transform · LLM extract","instructor + LiteLLM","taxonomies + complementary taxonomies")}{ARROW}
<div class="row">{node("target","Target state","Product node","declare_record · id · title · price")}{node("data","Data · carried forward","taxonomy labels","what it is · what pairs with it")}</div></div></div>{ARROW}
<div class="comp"><div class="comp-cap">Phase 2 &middot; Processing Component &middot; <span class="reg">mount &rarr; build_graph (one pass)</span></div><div class="col">
{node("logic","Transform","dedupe taxonomy labels","distinct Taxonomy set")}{ARROW}
{node("target","Target states","Taxonomy · PRODUCT_TAXONOMY · PRODUCT_COMPLEMENTARY_TAXONOMY","declare_record · declare_relation")}</div></div>{ARROW}
{node("logic","Target","Neo4j","product graph · Product · Taxonomy · edges")}</div>
