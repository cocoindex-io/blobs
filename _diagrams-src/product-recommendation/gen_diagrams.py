#!/usr/bin/env python3
"""V1 diagrams for the `product-recommendation` example.

Fitted to examples/product_recommendation/main.py — a TWO-phase pipeline:
  walk_dir(product JSON) -> [phase 1: process_file (per product): LLM extract
  taxonomies -> Product node, taxonomy labels carried forward] -> [phase 2:
  build_graph (one pass): dedupe labels -> Taxonomy nodes + relationship
  types] -> Neo4j.
...
def cover_html():
    ...
    pipe=sep.join([chip("Products"),chip("LLM"),chip("Taxonomies"),chip("Neo4j")])
...
<div><h1>Product <em>Recommendation</em><br>Graph</h1><div class="sub">LLM-extract what each product is and what pairs with it, into a Neo4j graph that powers recommendations.</div></div>
...
def flow_svg():
    ...
    s.append(f'<text x="82" y="{y+11}" dominant-baseline="central" font-family="{MONO}" '
             ...
             f'COCOINDEX · PRODUCT RECOMMENDATION FLOW</text>')
    y += 30 + 28
    ...
    s.append(fcomp(CX, y, COMP_W, COMP_H, "Phase 1 · Processing component",
                   "use_mount → process_file (per product)"))
    iy = y + COMP_PAD_T
    s.append(fnode(CX, iy, INNER_W, "logic", "Transform · LLM extract",
                   "instructor + LiteLLM",
                   "taxonomies + complementary taxonomies"))
    iy += FNODE_H
    s.append(vdown(CX, iy)); iy += VGAP
    s.append(fnode(CX - ROW_W / 2 - 15, iy, ROW_W, "target", "Target state",
                   "Product node", "declare_record · id · title · price"))
    s.append(fnode(CX + ROW_W / 2 + 15, iy, ROW_W, "data", "Data · carried forward",
                   "taxonomy labels", "what it is · what pairs with it"))
    y += COMP_H
    s.append(vdown(CX, y)); y += VGAP

    # ── Phase 2 ──
    s.append(fcomp(CX, y, COMP_W, COMP_H, "Phase 2 · Processing component",
                   "mount → build_graph (one pass)"))
    iy = y + COMP_PAD_T
    s.append(fnode(CX, iy, INNER_W, "logic", "Transform", "dedupe taxonomy labels",
                   "distinct Taxonomy set", mono=False))
    iy += FNODE_H
    s.append(vdown(CX, iy)); iy += VGAP
    s.append(fnode(CX, iy, INNER_W, "target", "Target states",
                   "Taxonomy · PRODUCT_TAXONOMY · PRODUCT_COMPLEMENTARY_TAXONOMY",
                   "declare_record · declare_relation", size=21))
    y += COMP_H
    s.append(vdown(CX, y)); y += VGAP

    s.append(fnode(CX, y, NODE_W, "logic", "Target", "Neo4j",
                   "product graph · Product · Taxonomy · edges", mono=False))
    y += FNODE_H
