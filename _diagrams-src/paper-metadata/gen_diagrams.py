...
Renders:
  ...
  stage-file-process.png per-file transform chain + 3 targets highlighted
...
.fan{{display:flex;align-items:flex-start;justify-content:center;gap:26px;}}
.fan .col{{flex:1;}}
...
.sw.comp{{background:color-mix(in oklab,{PEACH} 13%,{CREAM});border-color:{CORAL};border-radius:8px;}}
  ...
  <div class="eyebrow"><span class="dot"></span>CocoIndex &middot; Paper metadata flow</div>

  <div class="col">
    {node("logic", "Source", "localfs.walk_dir", "Local filesystem &middot; *.pdf &middot; watches for changes (live=True)")}
    {ARROW}
    {node("data", "Data", "files", "one FileLike per PDF")}
    {ARROW}

    <div class="comp">
      <div class="comp-cap">Processing Component &middot; <span class="reg">mount_each &rarr; process_file (per file, memoized)</span></div>
      <div class="col">
        {node("logic", "Transform &middot; pypdf", "extract_basic_info", "PDF bytes &rarr; first page + page count")}
        {ARROW}
        {node("logic", "Transform &middot; pypdf", "pdf_to_markdown", "first page &rarr; text")}
        {ARROW}
        {node("logic", "Transform &middot; LLM (gpt-4o)", "extract_metadata", "text &rarr; title &middot; authors &middot; abstract (JSON)", hl=True)}
        {ARROW}
        {node("data", "Data", "PaperMetadataModel", "title &middot; list[Author] &middot; abstract")}
        {ARROW}
        <div class="subcomp">
          <div class="subcomp-cap">embed title + abstract chunks &middot; declare rows into 3 tables</div>
          <div class="fan">
            <div class="col">
              {node("target", "Target state", "PaperMetadataRow", "title &middot; authors &middot; abstract &middot; pages")}
            </div>
            <div class="col">
              {node("target", "Target state", "AuthorPaperRow", "author_name &middot; filename")}
            </div>
            <div class="col">
              {node("target", "Target state", "MetadataEmbeddingRow", "location &middot; text &middot; embedding (384-d)")}
            </div>
          </div>
        </div>
      </div>
    ...
    {ARROW}
    {node("logic", "Target", "Postgres &middot; pgvector", "paper_metadata &middot; author_papers &middot; metadata_embeddings")}
  </div>
...
SVG_DIAGRAMS = {
    ...
    "stage-main-function": lambda: fanout_svg(hl_file=True, hl_pc=True),
}
svg_sizes = {}
...
if "--render" in sys.argv:
    ...
    subprocess.run([
        chrome, "--headless=new", "--hide-scrollbars", "--no-sandbox",
        "--force-color-profile=srgb", "--virtual-time-budget=8000",
        f"--screenshot={OUT_DIR / 'flow-v1.png'}", "--window-size=1240,1860",
