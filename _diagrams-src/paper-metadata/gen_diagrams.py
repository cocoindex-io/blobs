#!/usr/bin/env python3
"""V1 diagrams for the `paper-metadata` example.

Adapted from _diagrams-src/pdf-embedding/gen_diagrams.py, fitted to
examples/paper_metadata/main.py:
  walk_dir(*.pdf) -> files -> [process_file (per file, memoized):
  extract_basic_info -> pdf_to_markdown (first page) -> extract_metadata (LLM, gpt-4o)
  -> declare PaperMetadataRow + AuthorPaperRow(s) + embed(title/abstract chunks)
  -> declare_row(MetadataEmbeddingRow)] -> Postgres (3 tables, pgvector).
...
Renders (SVG is what the docs page embeds; HTML files are local previews):
  ...
  stage-file-process.svg   per-file transform chain + 3 targets highlighted
  stage-main-function.svg  file fan-out + process_file highlighted
...
def cover_html():
    ...
    pipeline = sep.join([chip("PDF"), chip("First page"), chip("LLM extract"),
                         chip("Embed"), chip("Postgres")])
...
.coco{{position:absolute;top:60px;right:78px;font-size:60px;}}
  ...
  <div>
    <h1>Structured <em>metadata</em><br>from PDF papers</h1>
    <div class="sub">Extract title, authors, and abstract with an LLM, embed them for semantic search, and store it all in Postgres — incremental, in plain async Python.</div>
  </div>
...
def fanout_svg(*, hl_fn=False, hl_file=False, hl_pc=False):
    APP_X = 250; APP_Y = 40; APP_H = 600
    PADX = 30; FILE_W = 170; FILE_H = 52; GAPF = 40
    PC_PADX = 28; XFM_W = 320; XFM_H = 92; GAPX = 40; OUT_W = 232; OUT_H = 56
    OUT_GAP = 26
    PCW = PC_PADX + XFM_W + GAPX + OUT_W + PC_PADX; PCH = 3 * OUT_H + 2 * OUT_GAP + 64
    ...
    OUT_ROWS = [("PaperMetadata", "row"), ("AuthorPaper", "rows"),
                ("MetadataEmbedding", "rows")]

    s = [DEFS]
    s.append(logic(*SRC, ["PDFs", "folder"], size=26, sub="walk_dir · live"))
    s.append(app(APP_X, APP_Y, APP_W, APP_H, "CocoIndex App"))
    s.append(logic(TGT_X, APP_Y, TGT_W, APP_H, ["Postgres"], size=26, sub="3 tables · pgvector"))

    for cy, fname in [(202, "attention.pdf"), (478, "bert.pdf")]:
        s.append(bind(SRC[0] + SRC[2], cy, file_x))
        s.append(data(file_x, cy - FILE_H/2, FILE_W, FILE_H, [fname], hl=hl_file, size=19))
        ...
        s.append(logic(xfm_x, cy - XFM_H/2, XFM_W, XFM_H,
                       ["LLM extract", "+ embed"], hl=hl_fn, size=24))
        block_h = 3 * OUT_H + 2 * OUT_GAP
        y0 = cy - block_h / 2
        for i, (l1, l2) in enumerate(OUT_ROWS):
            oy = y0 + i * (OUT_H + OUT_GAP)
            ocy = oy + OUT_H / 2
            s.append(arrow(xfm_x + XFM_W, ocy, out_x))
            s.append(bullet(out_x, oy, OUT_W, OUT_H, [l1, l2], hl=hl_fn, size=17))
            s.append(bind(out_x + OUT_W, ocy, TGT_X))

    ex = APP_X + APP_W / 2
    for dy in (-20, 0, 20):
        s.append(f'<circle cx="{ex}" cy="{APP_Y + APP_H - 30 + dy}" r="6" fill="{MUTED}"/>')
    return W, H, "".join(s)
...
# peach = component, dashed = fan-out sub-component).
...
def fnode(cx, y, w, kind, cap, ttl, sub="", *, mono=True, hl=False, h=FNODE_H,
    ...
    ts = ttl_size if ttl_size else (24 if mono else 26)
    s.append(_text(cx, y + h / 2 + 4, ttl, size=ts, mono=mono))
    if sub:
        s.append(_text(cx, y + h - 20, sub, size=sub_size, weight=400, color=MUTED))
    return "".join(s)
...
def flow_svg():
    ...
    FAN_GAP = 26
    FAN_W = (SUB_INNER_W - 2 * FAN_GAP) / 3

    SUB_H = SUB_PAD_T + FNODE_H + SUB_PAD_B
    COMP_H = (COMP_PAD_T + 4 * (FNODE_H + VGAP) + SUB_H + COMP_PAD_B)
    ...
    s.append(f'<text x="82" y="{y+11}" dominant-baseline="central" font-family="{MONO}" '
             ...
             f'COCOINDEX · PAPER METADATA FLOW</text>')
    y += 30 + 28

    s.append(fnode(CX, y, NODE_W, "logic", "Source", "localfs.walk_dir",
                   "Local filesystem · *.pdf · watches for changes (live=True)"))
    y += FNODE_H
    s.append(vdown(CX, y)); y += VGAP

    s.append(fnode(CX, y, NODE_W, "data", "Data", "files", "one FileLike per PDF"))
    y += FNODE_H
    s.append(vdown(CX, y)); y += VGAP
    ...
    iy = y + COMP_PAD_T
    s.append(fnode(CX, iy, INNER_W, "logic", "Transform · pypdf", "extract_basic_info",
                   "PDF bytes → first page + page count"))
    iy += FNODE_H
    s.append(vdown(CX, iy)); iy += VGAP
    s.append(fnode(CX, iy, INNER_W, "logic", "Transform · pypdf", "pdf_to_markdown",
                   "first page → text"))
    iy += FNODE_H
    s.append(vdown(CX, iy)); iy += VGAP
    s.append(fnode(CX, iy, INNER_W, "logic", "Transform · LLM (gpt-4o)", "extract_metadata",
                   "text → title · authors · abstract (JSON)", hl=True))
    iy += FNODE_H
    s.append(vdown(CX, iy)); iy += VGAP
    s.append(fnode(CX, iy, INNER_W, "data", "Data", "PaperMetadataModel",
                   "title · list[Author] · abstract"))
    iy += FNODE_H
    s.append(vdown(CX, iy)); iy += VGAP

    s.append(fsubcomp(CX, iy, SUB_W, SUB_H,
                      "embed title + abstract chunks · declare rows into 3 tables"))
    fy = iy + SUB_PAD_T
    fan = [("PaperMetadataRow", "title · authors · abstract · pages"),
           ("AuthorPaperRow", "author_name · filename"),
           ("MetadataEmbeddingRow", "location · text · embedding (384-d)")]
    fx = CX - SUB_INNER_W / 2 + FAN_W / 2
    for ttl, sub in fan:
        s.append(fnode(fx, fy, FAN_W, "target", "Target state", ttl, sub,
                       ttl_size=19, sub_size=14))
        fx += FAN_W + FAN_GAP
    y += COMP_H
    s.append(vdown(CX, y)); y += VGAP

    s.append(fnode(CX, y, NODE_W, "logic", "Target", "Postgres · pgvector",
                   "paper_metadata · author_papers · metadata_embeddings"))
    y += FNODE_H
...
# obsolete pure-CSS flow preview, replaced by flow-v1.svg/.html
(OUT_DIR / "flow.html").unlink(missing_ok=True)
