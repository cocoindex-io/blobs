...
Renders:
  ...
  stage-file-process.png per-form fan-out: one process_patient_form component per PDF
...
.sw.comp{{background:color-mix(in oklab,{PEACH} 13%,{CREAM});border-color:{CORAL};border-radius:8px;}}
  ...
  <div class="eyebrow"><span class="dot"></span>CocoIndex &middot; Patient intake extraction flow</div>

  <div class="col">
    {node("logic", "Source", "localfs.walk_dir", "Local filesystem · **/*.pdf intake forms")}
    {ARROW}
    {node("data", "Data", "files", "one FileLike per intake PDF")}
    {ARROW}

    <div class="comp">
      <div class="comp-cap">Processing Component &middot; <span class="reg">mount_each &rarr; process_patient_form (per file, memoized)</span></div>
      <div class="col">
        {node("logic", "Transform · render + extract", "extract_patient", "PyMuPDF pages → DSPy ChainOfThought (Gemini) → Patient", hl=True)}
        {ARROW}
        {node("target", "Target state · one per form", "declare_file → &lt;stem&gt;.json", "validated Patient · model_dump_json(indent=2)", hl=True)}
      </div>
    </div>

    {ARROW}
    {node("logic", "Target", "Local directory", "output_patients/ · one JSON per intake form")}
  </div>
