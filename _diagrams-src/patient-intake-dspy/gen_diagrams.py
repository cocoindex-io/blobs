#!/usr/bin/env python3
"""V1 diagrams for the `patient-intake-dspy` example.

Adapted from _diagrams-src/image-search and text-embedding generators, fitted to
examples/patient_intake_extraction_dspy/main.py:
  localfs.walk_dir(*.pdf) -> files -> [process_patient_form (per file, memoized):
  PyMuPDF render pages -> DSPy ChainOfThought vision (Gemini) -> Patient ->
  declare_file(<stem>.json)] -> local output directory.
...
Renders (SVG is what the docs page embeds; HTML files are local previews):
  ...
  stage-file-process.svg per-form fan-out: one process_patient_form component per PDF
...
def cover_html():
    ...
    pipeline = sep.join([chip("Intake PDF"), chip("Page images"), chip("DSPy &middot; Gemini"), chip("Patient JSON")])
...
.coco{{position:absolute;top:60px;right:78px;font-size:60px;}}
  ...
  <div>
    <h1>Patient Intake Forms<br>to <em>Structured JSON</em></h1>
    <div class="sub">Render each intake PDF to images and let a DSPy vision module on Gemini extract a typed Patient — one validated JSON record per form.</div>
  </div>
...
def fanout_svg(*, hl_fn=True):
    ...
    PADX = 30; FILE_W = 152; FILE_H = 52; GAPF = 40
    PC_PADX = 28; XFM_W = 320; XFM_H = 92; GAPX = 36; OUT_W = 196; OUT_H = 56
    ...
    s = [DEFS]
    s.append(logic(*SRC, ["Intake", "forms"], size=26, sub="walk_dir · *.pdf"))
    s.append(app(APP_X, APP_Y, APP_W, APP_H, "CocoIndex App"))
    s.append(logic(TGT_X, APP_Y, TGT_W, APP_H, ["Local dir"], size=26, sub="output_patients/"))
    ...
    for cy, fname in [(160, "David.pdf"), (340, "Emily.pdf")]:
        ...
        s.append(pc(pc_x, cy - PCH/2, PCW, PCH, "process_patient_form", hl=hl_fn))
        s.append(logic(xfm_x, cy - XFM_H/2 + 10, XFM_W, XFM_H,
                       ["render → DSPy extract"], hl=hl_fn, size=23))
        s.append(arrow(xfm_x + XFM_W, cy, out_x))
        s.append(bullet(out_x, cy - OUT_H/2, OUT_W, OUT_H,
                        ["Patient JSON"], hl=hl_fn, size=18))
...
def flow_svg():
    ...
    s.append(fcomp(CX, y, COMP_W, COMP_H, "Processing component",
                   "mount_each → process_patient_form (per file, memoized)"))
    iy = y + COMP_PAD_T
    s.append(fnode(CX, iy, INNER_W, "logic", "Transform · render + extract",
                   "extract_patient",
                   "PyMuPDF pages → DSPy ChainOfThought (Gemini) → Patient", hl=True))
    iy += FNODE_H
    s.append(vdown(CX, iy)); iy += VGAP
    s.append(fnode(CX, iy, INNER_W, "target", "Target state · one per form",
                   "declare_file → &lt;stem&gt;.json",
                   "validated Patient · model_dump_json(indent=2)", hl=True))
    y += COMP_H
    s.append(vdown(CX, y)); y += VGAP

    s.append(fnode(CX, y, NODE_W, "logic", "Target", "Local directory",
                   "output_patients/ · one JSON per intake form"))
    y += FNODE_H
